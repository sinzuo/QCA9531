        ...
        ath_bufhead *bf_head, ieee80211_tx_control_t *txctl)
{
    ...
    if (sc->sc_atf_enable)
    {
        ...
        if ( !ath_atf_bypass_frame( tid, bf ) && !(tid->an->an_atf_txtoken) )
        {
    ...
    if(sc->sc_atf_enable)
    {
        ...
        {
            /* Set node tokens to '0', if tokens not available. */
            tid->an->an_atf_nomore_tokens = 1;
            tid->an->an_atf_txtoken = 0;
        }
    }
#endif
    if (!sc->sc_nodebug) {
...
ath_tx_sched_aggr(struct ath_softc *sc, struct ath_txq *txq, ath_atx_tid_t *tid)
{
    ...
    do {
        ...
        if ( sc->sc_atf_enable ) {
            ...
            if ( !ath_atf_bypass_frame(tid, bft ) && !(tid->an->an_atf_txtoken) ) {
                /* Do not dequeue if tokens not available. */
                ath_tx_queue_tid(txq, tid);
                break;
            }
