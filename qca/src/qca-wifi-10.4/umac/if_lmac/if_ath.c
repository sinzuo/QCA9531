...
static void
ath_bsteering_rssi_update(ieee80211_handle_t ieee, u_int8_t *sta_mac_addr, u_int8_t tx_status,int8_t rssi,u_int8_t subtype) 
{
    struct ieee80211com *ic = NET80211_HANDLE(ieee);
...
void da_reset_wifi(struct ath_softc *sc,struct ieee80211com *ic)
{
    ...
    if (init_sc_params(sc,sc->sc_ah))
        adf_os_print("%s init sc params failed\n",__func__);
