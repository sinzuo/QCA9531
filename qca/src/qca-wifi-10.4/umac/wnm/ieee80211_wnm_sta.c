                                ...
                                u_int8_t *frm, int frm_len)
{
    ...
    event = adf_os_mem_alloc(NULL, sizeof(wnm_netlink_event_t));
    if (event == NULL) {
        printk("%s[%d]: Memory allocation failed\n", __func__, __LINE__);
        return -ENOMEM;
    }

    fmsrsp_msg = adf_os_mem_alloc(NULL, sizeof(struct ieee80211_wnm_fmsresp));
    if (fmsrsp_msg == NULL) {
        printk("%s[%d]: Memory allocation failed\n", __func__, __LINE__);
        adf_os_mem_free(event);
        return -ENOMEM;
    }
                                ...
                                u_int8_t *frm, int frm_len)
{
    ...
    tfsrsp = adf_os_mem_alloc(NULL, sizeof(struct ieee80211_wnm_tfsresp));
    if (tfsrsp == NULL) {
        printk("%s[%d]: Memory allocation failed\n", __func__, __LINE__);
        adf_os_mem_free(event);
        return -ENOMEM;
