            ...
            if(ni_mbo && ni_mbo->num_attr) {
                for(attr_index = 0;attr_index< ni_mbo->num_attr;attr_index++) {
                    if(ni_mbo->channel[attr_index].channels_preference == 0
                       || ni_mbo->channel[attr_index].channels_preference == 1) {
                                     ...
                                     uint8_t validity_interval)
{
    ...
    *frm = &bstm_req->opt_ies[0];
    ...
    if (bstm_req->mode_term_inc) {
        ...
        btdur->len = sizeof(struct ieee80211_bssterm_duration);
        btdur->tsf = htole64(vap->wnm->wnm_bssterm_tsf);
        btdur->duration = htole16(vap->wnm->wnm_bssterm_duration);
        *frm += sizeof(*btdur);
    }
