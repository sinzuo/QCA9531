...
#if ATH_SUPPORT_WRAP
#ifdef IEEE80211_DEBUG_REFCNT
    return _wrap_find_rxnode_debug(nt, wh->i_addr1, wh->i_addr2, __func__, __LINE__);
#else
...
static void ieee80211_timeout_node_saveq_age( struct ieee80211_node_table *nt)
{
  ...
  itr_arg = adf_os_mem_alloc(NULL, sizeof(struct ieee80211_iter_arg));
  if (itr_arg == NULL) {
          return;
  }
...
void
ieee80211_mu_cap_client_join_leave(struct ieee80211_node *ni,const u_int8_t type)
{
    struct    ieee80211vap *vap = ni->ni_vap;
    u_int8_t  i;
    ...
    if(type)
    {
      /*Client joining*/
       if (ni->ni_mu_vht_cap)
       {
          OS_MEMCPY(vap->mu_cap_client_addr[vap->mu_cap_client_num+vap->mu_cap_dedicate_client_num],
                    (char *)(ni->ni_macaddr),
                    IEEE80211_ADDR_LEN);

          if(!ni->ni_mu_dedicate_fg)
          {
            vap->mu_cap_client_flag[vap->mu_cap_client_num+vap->mu_cap_dedicate_client_num] = 1;
            /*mu-cap clients without dedicated client*/
            if(vap->mu_cap_war == 1)
            {
                if((vap->mu_cap_client_num == 0) && (vap->iv_mu_cap_state_switch == MU_CAP_STATE_NOR))
                     vap->iv_mu_cap_state_switch =  MU_CAP_STATE_CHG;
            }
            vap->mu_cap_client_num++;
          }else{
            /*mu-cap dedicated clients*/
            vap->mu_cap_client_flag[vap->mu_cap_client_num+vap->mu_cap_dedicate_client_num] = 0;
            adf_os_spin_lock_bh(&vap->iv_mu_cap_lock);
            vap->mu_cap_dedicate_client_num++;
            adf_os_spin_unlock_bh(&vap->iv_mu_cap_lock);
          }
       }
    }else{
       /*Client leaving*/
       for (i = 0; i < (vap->mu_cap_client_num + vap->mu_cap_dedicate_client_num); i++)
       {
          if (IEEE80211_ADDR_EQ((char *)(vap->mu_cap_client_addr[i]), (char *)(ni->ni_macaddr)))
          {
               OS_MEMCPY((char *)(vap->mu_cap_client_addr[i]),
                         (char *)(vap->mu_cap_client_addr[vap->mu_cap_client_num + vap->mu_cap_dedicate_client_num-1]),
                         IEEE80211_ADDR_LEN);
               adf_os_mem_set(&(vap->mu_cap_client_addr[vap->mu_cap_client_num + vap->mu_cap_dedicate_client_num-1][0]),0,IEEE80211_ADDR_LEN);
               if(vap->mu_cap_client_flag[i] == 1 )
               {
                 if(vap->mu_cap_war == 1)
                 {
                    if ((vap->mu_cap_client_num == 1) && (vap->iv_mu_cap_state_switch == MU_CAP_STATE_NOR))
                       vap->iv_mu_cap_state_switch =  MU_CAP_STATE_CHG;
                 }

                 OS_MEMCPY((char *)(&(vap->mu_cap_client_flag[i])),
                         (char *)(&(vap->mu_cap_client_flag[vap->mu_cap_client_num + vap->mu_cap_dedicate_client_num-1])),
                         1);
                 vap->mu_cap_client_flag[vap->mu_cap_client_num + vap->mu_cap_dedicate_client_num-1] = 0;
                 vap->mu_cap_client_num--;
               }else{
                 if( vap->mu_cap_dedicate_client_num >0)
                 {
                     OS_MEMCPY((char *)(&(vap->mu_cap_client_flag[i])),
                         (char *)(&(vap->mu_cap_client_flag[vap->mu_cap_client_num + vap->mu_cap_dedicate_client_num-1])),
                         1);
    ...
    if ((vap->iv_mu_timer_state != MU_TIMER_RUNNING) && (vap->mu_cap_war == 1))
    {
        if (vap->iv_mu_cap_state_switch == MU_CAP_STATE_CHG)
        {
             adf_os_spin_lock_bh(&vap->iv_mu_cap_lock);
             vap->iv_mu_timer_state = MU_TIMER_RUNNING;
             OS_SET_TIMER(&vap->iv_mu_cap_timer,vap->mu_cap_timer_period*1000);
             adf_os_spin_unlock_bh(&vap->iv_mu_cap_lock);
        }
    }
}
