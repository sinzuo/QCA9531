...
ieee80211_disable_legacy_rates(struct ieee80211vap *vap)
{
    ...
    if (vap->iv_bcn_rate) {
        ...
        if (found) {
            ...
            adf_os_print("%s:MSG: User has disabled the previously set modified rate for beacon.\n",__func__);
            adf_os_print("%s:MSG: Hence going for the lowest available basic rate for beacon: %d (Kbps)\n",__func__,vap->iv_bcn_rate);
        }
