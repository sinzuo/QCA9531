    ...
    *(u_int16_t *)frm = htole16(capinfo);
    ...
    if (ieee80211_vap_wme_is_set(vap) &&
        ...
        if (ASSOCWAR160_IS_VHT_CAP_CHANGE(vap->iv_cfg_assoc_war_160w))
            frm = ieee80211_add_vhtcap(frm, ni, ic, IEEE80211_FC0_SUBTYPE_PROBE_RESP, extractx);
        else
            frm = ieee80211_add_vhtcap(frm, ni, ic, IEEE80211_FC0_SUBTYPE_PROBE_RESP, NULL);
...
void write_mac_to_vap (struct ieee80211vap *vap)
{

    struct ieee80211_stats *st = &(vap->iv_stats);
    int i;
   // unsigned char tmp[6];
    int total=0;
    int num=0;
    int average=0;
    unsigned int lastpriv=0,firstpriv=0;

    memset(st->is_probsta,0,sizeof(st->is_probsta));
    st->is_probsta_count =0;

    for(i=1;i<NUM_MAC_TABLE;i++)
    {
        if(memcmp(mactable[i], mactable[firstpriv], 6) != 0) 

        {
        lastpriv=i-1;
        num=lastpriv-firstpriv+1;
           for(;firstpriv<=lastpriv;firstpriv++)
           {
            total+=mactable[firstpriv][6];
           }

           average=total/num;
           
           memcpy(st->is_probsta[st->is_probsta_count], mactable[lastpriv], 6);
           st->is_probsta[st->is_probsta_count][6]= ( unsigned char)average;
    
          st->is_probsta_count ++;
        if(NUM_MAC_TABLE==st->is_probsta_count)
        {
        printk("200 is not enough for mac table\n");
        break;
        }
          total=0;
          num=0;
        ...
        struct ieee80211_ie_vhtcap *sta_vhtcap)
{
    ...
    adf_os_assert_always(vap != NULL);
    adf_os_assert_always(probereq_ssid_ie != NULL);
                        ...
                        struct ieee80211_rx_status *rs)
{
    ...
    (void) rs;
#endif
    adf_os_mem_set((char *)(ni->ni_dedicated_macaddr),0,IEEE80211_ADDR_LEN);
    /*
     * prreq frame format
     *  [tlv] ssid
     *  [tlv] supported rates
    ...
    while (((frm+1) < efrm) && (frm + frm[1] + 1 < efrm)) {
        ...
        case IEEE80211_ELEMID_VENDOR:
           ...
           {
              ni->ni_mu_bfee_cap = 1;
              OS_MEMCPY((char *)(ni->ni_dedicated_macaddr),(char *)(wh->i_addr2),IEEE80211_ADDR_LEN);
           }else{
              ni->ni_mu_bfee_cap = 0;
           }
            break;
