...
u_int8_t* add_chan_switch_ie(u_int8_t *frm, struct ieee80211_channel *next_ch)
{
    ...
    csaie->tbttcount = IEEE80211_RADAR_11HCOUNT;
...
ieee80211_add_next_channel(u_int8_t *frm, struct ieee80211_node *ni, struct ieee80211com *ic, int subtype)
{
    ...
    frm = add_chan_switch_ie(frm, next_channel);
    ...
    )
{
   ...
   if(widebwie) {
    ...
    if (dest_chwidth > max_allowed_chwidth) {
                     ...
                     struct ieee80211_framing_extractx *extractx)
{
    ...
    if(scn->target_type == TARGET_TYPE_QCA9984 || scn->target_type == TARGET_TYPE_QCA9888) {
        vhtcap_info &= ((vap->iv_sgi) ? ic->ic_vhtcap : ~(IEEE80211_VHTCAP_SHORTGI_80 | IEEE80211_VHTCAP_SHORTGI_160));
    }
    else {
    ...
    if ((subtype ==  IEEE80211_FC0_SUBTYPE_PROBE_RESP) && (vap->mu_cap_war == 1))
    {
        if(vap->mu_cap_client_num !=0)
        {
             if (ni->ni_mu_bfee_cap)
             {
                     ...
                     struct ieee80211com *ic, u_int8_t subtype)
{
    ...
    ieee80211_add_vhtcap(frm + 7 , ni, ic, subtype, NULL); /* Vht IE location Inside Vendor specific VHT IE*/
                    ...
                    struct ieee80211com *ic,  u_int8_t subtype, int is_vendor_ie)
{
    ...
    switch(next_chwidth)
    {
        ...
        case CHWIDTH_VHT160:
            if (IEEE80211_IS_CHAN_11AC_VHT160(next_channel)) {
                ...
                printk("%s: Error: chanwidth 160 requested for a channel"
                       " which is neither VHT60 nor VHT80_80\n", __func__);
    ...
    if(new_ch_width != IEEE80211_VHTOP_CHWIDTH_2040) {
       ...
       if(new_ch_width == IEEE80211_VHTOP_CHWIDTH_80_80) {
         ...
         */
       if ((new_ch_width == IEEE80211_VHTOP_CHWIDTH_2040) &&
                ((ic->ic_curmode == IEEE80211_MODE_11AC_VHT80) || (ic->ic_curmode == IEEE80211_MODE_11AC_VHT160)
                 || (ic->ic_curmode == IEEE80211_MODE_11AC_VHT80_80) )) {

           new_phy_mode = ieee80211_chan2mode(next_channel);

           if(new_phy_mode == IEEE80211_MODE_11AC_VHT40PLUS) {
               widebw->new_ch_freq_seg1 = next_chan+2;
