                      ...
                      u_int8_t *frm)
{
    ...
    *frm++ = IEEE80211_ELEMID_SSID;
    ...
    if (ieee80211_vap_wme_is_set(vap) && 
        ...
        bo->bo_vhtcap = frm;
        frm = ieee80211_add_vhtcap(frm, ni, ic, IEEE80211_FC0_SUBTYPE_BEACON, NULL);
    ...
    if(ieee80211_vap_mbo_check(vap)) {
        bo->bo_mbo_cap = frm;
        frm = ieee80211_setup_mbo_ie(vap,frm);
    } else {
        bo->bo_mbo_cap = NULL;
    }
...
#endif
{
    ...
    *(u_int16_t *)&wh->i_seq[0] =
    ...
    if (vap->iv_update_vendor_ie ) {
        adf_os_print("%s reinit beacon as vndor ie confguration chnaged \n",__func__);
    ...
    */
    ...
    if ((vap->iv_flags & IEEE80211_F_CHANSWITCH) &&
        (vap->iv_chanchange_count == ic->ic_chanchange_tbtt) && IEEE80211_CHANCHANGE_BY_BEACONUPDATE_IS_SET(ic)
            ...
            if(!(IEEE80211_IS_CHAN_DFS(c) ||
                ...
                TAILQ_FOREACH(tmp_vap, &ic->ic_vaps, iv_next) {
                    if(tmp_vap->iv_opmode == IEEE80211_M_HOSTAP ||
                        ...
                        } else if(tmp_vap->iv_opmode == IEEE80211_M_STA) {
                    ...
                    TAILQ_FOREACH(tmp_vap, &ic->ic_vaps, iv_next) {
                        if(tmp_vap->iv_opmode == IEEE80211_M_HOSTAP ||
                            ...
                            if (ieee80211_vap_ready_is_set(tmp_vap)){
                                ...
                                tmp_vap->iv_chanchange_count = 0;
                                tmp_vap->iv_flags &= ~IEEE80211_F_CHANSWITCH;
                                if (IEEE80211_IS_CSH_CAC_APUP_BYSTA_ENABLED(ic)) {
                                    tmp_vap->iv_no_cac = 0;
                                } else {
                                    /* If Repeater CAC is set to 0 by the user, skip AP vap cac
                                     * when Repeater STA vap is connecting to RootAP.
                                     */
                                    tmp_vap->iv_no_cac = 1;
                                }
                                channel_switch_set_channel(tmp_vap, ic);
    ...
    *bo->bo_caps = htole16(capinfo);
    ...
    if (ieee80211_vap_wme_is_set(vap) && 
        ...
        /* Add VHT capabilities IE */
        ieee80211_add_vhtcap(bo->bo_vhtcap, ni, ic, IEEE80211_FC0_SUBTYPE_BEACON, NULL);
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP || vap->iv_opmode == IEEE80211_M_IBSS) {
        ...
        if (ieee80211_ic_doth_is_set(ic) && (ic->ic_flags & IEEE80211_F_CHANSWITCH) && (ic->ic_chanchange_channel) &&
            ...
            if (!vap->iv_chanchange_count) {
                ...
                if (bo->bo_apriori_next_channel) {
                    bo->bo_apriori_next_channel += IEEE80211_CHANSWITCHANN_BYTES;
                }
                bo->bo_appie_buf += IEEE80211_CHANSWITCHANN_BYTES;
                if(bo->bo_whc_apinfo) {
                    bo->bo_whc_apinfo += IEEE80211_CHANSWITCHANN_BYTES;
                }
                bo->bo_xr += IEEE80211_CHANSWITCHANN_BYTES;
#ifdef E_CSA
                bo->bo_extchanswitch += IEEE80211_CHANSWITCHANN_BYTES;
#endif /* E_CSA */
                bo->bo_secchanoffset += IEEE80211_CHANSWITCHANN_BYTES;
                if(bo->bo_htinfo != NULL && bo->bo_htcap != NULL)
                {
                ...
                if ( ieee80211_vap_mbo_check(vap) && (bo->bo_mbo_cap) ) {
                    bo->bo_mbo_cap += IEEE80211_CHANSWITCHANN_BYTES;
                }
...
#endif /* ATH_SUPPORT_IBSS_DFS */
#ifdef E_CSA
                        bo->bo_extchanswitch_trailerlen += IEEE80211_SEC_CHAN_OFFSET_BYTES;
#endif
    ...
    if (ieee80211_ic_doth_is_set(ic) && (ic->ic_flags & IEEE80211_F_CHANSWITCH)&& IEEE80211_CHANCHANGE_BY_BEACONUPDATE_IS_SET(ic)) {
        ...
        if (!vap->iv_chanchange_count)  {
            ...
            bo->bo_secchanoffset +=IEEE80211_EXTCHANSWITCHANN_BYTES;
#if ATH_SUPPORT_IBSS_DFS
            ...
            if (bo->bo_bwnss_map != NULL) {
                bo->bo_bwnss_map += IEEE80211_EXTCHANSWITCHANN_BYTES;
            }
            bo->bo_ath_caps += IEEE80211_EXTCHANSWITCHANN_BYTES;
            if (bo->bo_apriori_next_channel) {
                bo->bo_apriori_next_channel += IEEE80211_EXTCHANSWITCHANN_BYTES;
            }
            bo->bo_xr += IEEE80211_EXTCHANSWITCHANN_BYTES;
            bo->bo_appie_buf += IEEE80211_EXTCHANSWITCHANN_BYTES;
            if(bo->bo_whc_apinfo) {
                bo->bo_whc_apinfo += IEEE80211_EXTCHANSWITCHANN_BYTES;
            }
#if ATH_SUPPORT_IBSS_DFS
            bo->bo_ibssdfs_trailerlen += IEEE80211_EXTCHANSWITCHANN_BYTES;
#endif
...
#endif /* UMAC_SUPPORT_WNM */
            if ( ieee80211_vap_mbo_check(vap) && (bo->bo_mbo_cap) ) {
                bo->bo_mbo_cap += IEEE80211_EXTCHANSWITCHANN_BYTES;
            }
	        ...
	        tempbuf = OS_MALLOC(vap->iv_ic->ic_osdev, bo->bo_vhtchnsw_trailerlen, GFP_KERNEL);
	            ...
	            OS_FREE(tempbuf);
                    if(vhtchnsw_ielen) {
                        ...
                        if (bo->bo_bwnss_map != NULL) {
                            bo->bo_bwnss_map += vhtchnsw_ielen;
                        }
                        bo->bo_tim_trailerlen += vhtchnsw_ielen;
                        bo->bo_chanswitch_trailerlen += vhtchnsw_ielen;
                        bo->bo_vhtchnsw_trailerlen += vhtchnsw_ielen;
