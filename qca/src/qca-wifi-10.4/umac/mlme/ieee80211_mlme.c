...
static void mlme_store_other_ic_stavap(struct ieee80211com *ic, struct ieee80211vap *vap)
{
    struct ieee80211com           *tmp_ic;
    int i;

    if(ic->other_ic_stavap) {
        return;
    }
    for (i=0; i < MAX_RADIO_CNT; i++) {
        ...
        if (tmp_ic && (ic != tmp_ic)) {
            if (tmp_ic->ic_sta_vap) {
                /*update other radio STA VAP in ic*/
                spin_lock(&ic->ic_lock);
                ic->other_ic_stavap = tmp_ic->ic_sta_vap;
                printk("ic->other_ic_stavap mac:%s\n",ether_sprintf(ic->other_ic_stavap->iv_myaddr));
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
                osif_nss_ol_store_other_pdev_stavap(ic);
#endif
                ic->ic_primary_radio = !tmp_ic->ic_primary_radio;
                spin_unlock(&ic->ic_lock);
        
                /*update STA VAP in other ic*/
                spin_lock(&tmp_ic->ic_lock);
                tmp_ic->other_ic_stavap = vap;
                printk("tmp_ic->other_ic_stavap mac:%s\n",ether_sprintf(tmp_ic->other_ic_stavap->iv_myaddr));
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
...
void wlan_mlme_connection_up(wlan_if_t vaphandle)
{
    ...
    && !vap_is_psta(vap)
        ...
        mlme_store_other_ic_stavap(ic, vap);
        ...
        if (ic->ic_global_list->num_stavaps_up == 2) {
            ...
            printk("\nBoth radio's STA VAPs are connected\n");
            if (ic->ic_global_list->dbdc_process_enable) {
                ...
                for (i=0; i < MAX_RADIO_CNT; i++) {
                    ...
                    if (tmp_ic) {
                        spin_lock(&tmp_ic->ic_lock);
                        osif_nss_ol_enable_dbdc_process(tmp_ic, tmp_ic->ic_global_list->dbdc_process_enable);
                        spin_unlock(&tmp_ic->ic_lock);
                    }
                }
#endif

                IEEE80211_DELIVER_EVENT_STA_VAP_CONNECTION_UPDATE(vaphandle);
            }
        }
