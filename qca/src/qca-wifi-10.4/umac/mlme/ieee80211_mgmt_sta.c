...
void ieee80211_mucap_vattach(struct ieee80211vap *vap) {
   struct ieee80211com *ic = vap->iv_ic;
   osdev_t os_handle = ic->ic_osdev;
   vap->iv_mu_cap_state_switch = MU_CAP_STATE_NOR;

   OS_INIT_TIMER(os_handle, &vap->iv_mu_cap_timer, ieee80211_mu_cap_mode_switch, vap);
}
...
static OS_TIMER_FUNC(ieee80211_mu_cap_mode_switch)
{
   ...
   cnt = old_num = vap->mu_cap_dedicate_client_num;
   while(cnt)
   {
      if((i < (vap->mu_cap_client_num + cnt))&&(vap->mu_cap_client_flag[i] == 0))
      {
          /*Found the dedicate client  nd ni entry, and then de-assoc */

          TAILQ_FOREACH(ni, &nt->nt_node, ni_list)
          {
            if(OS_MEMCMP((char *)(vap->mu_cap_client_addr[i]),ni->ni_macaddr,IEEE80211_ADDR_LEN) == 0)
            {
                if (ni->ni_associd != 0 )
                {
                     ieee80211_ref_node(ni);
                     associd = ni->ni_associd;
                     wlan_mlme_deauth_request(ni->ni_vap,ni->ni_macaddr,IEEE80211_REASON_UNSPECIFIED);
                     IEEE80211_DELIVER_EVENT_MLME_DEAUTH_INDICATION(ni->ni_vap, ni->ni_macaddr, associd, IEEE80211_REASON_UNSPECIFIED);
                     ieee80211_free_node(ni);
                }
                break;
            }
          }
          /*Removed from table and reduced counter*/
          OS_MEMCPY((char *)(vap->mu_cap_client_addr[i]),
                       (char *)(vap->mu_cap_client_addr[vap->mu_cap_client_num + cnt-1]),
                       IEEE80211_ADDR_LEN);
          OS_MEMCPY((char *)(&(vap->mu_cap_client_flag[i])),
                       (char *)(&(vap->mu_cap_client_flag[vap->mu_cap_client_num + cnt-1])),
                       1);
          memset(&(vap->mu_cap_client_addr[vap->mu_cap_client_num + cnt - 1][0]),0,IEEE80211_ADDR_LEN);
          vap->mu_cap_client_flag[vap->mu_cap_client_num + cnt - 1] = 0;
          cnt--;
      }else{
          i++;
      }
   }
   if (vap->mu_cap_dedicate_client_num >= old_num)
       vap->mu_cap_dedicate_client_num -= old_num;
   else
       vap->mu_cap_dedicate_client_num = 0;
   vap->iv_mu_cap_state_switch = MU_CAP_STATE_NOR;
   vap->iv_mu_timer_state = MU_TIMER_STOP;
   adf_os_spin_unlock_bh(&vap->iv_mu_cap_lock);
   if (vap->mu_cap_post_war == 1)
   {
       vap->mu_cap_war = 0;
       vap->mu_cap_post_war = 0;
   }
}
...
void ieee80211_mucap_vdetach(struct ieee80211vap *vap) {
   ...
   if (vap->iv_mu_timer_state == MU_TIMER_RUNNING)
      vap->iv_mu_timer_state = MU_TIMER_STOP;
   OS_FREE_TIMER(&vap->iv_mu_cap_timer);
   adf_os_spin_unlock_bh(&vap->iv_mu_cap_lock);
}
            ...
            if(chanie || echanie) {
                ...
                if (chan) {
                    ...
                    IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL, IEEE80211_MSG_EXTIOCTL_CHANSWITCH,
                            ...
                            chan->ic_ieee,  chan->ic_flags, __func__);
                }
                    ...
                    */
                    ...
                    if(IEEE80211_IS_CSH_IGNORE_CSA_DFS_ENABLED(ic) && IEEE80211_IS_CHAN_DFS(chan)) {
                        ...
                        if(target_channel != -1) {
                            ...
                            adf_os_print("%s: vap-%d(%s) channel is not available, bringdown all the AP vaps\n",
    ...
    )
{
    ...
    *(u_int16_t *)frm = htole16(ic->ic_lintval);
    ...
    if ((IEEE80211_IS_CHAN_11AC(ic->ic_curchan) || IEEE80211_IS_CHAN_11NG(ic->ic_curchan)) &&
        ...
        frm = ieee80211_add_vhtcap(frm, ni, ic, IEEE80211_FC0_SUBTYPE_ASSOC_REQ, NULL);
