...
static void ieee80211_mlme_create_infra_continue(struct ieee80211vap *vap)
{
    ...
    /* if station vap is present, and ap vap is not Repeater-Ind then skip DFS CAC timer */
    if ((vap_opmode_count.sta_count >= 1) && (!ieee80211_ic_enh_ind_rpt_is_set(vap->iv_ic))) {
       skip_dfs_cac = true;
    }
