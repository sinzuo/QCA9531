...
u_int32_t
ol_tx_pflow_enqueue_nawds(void *arg, adf_nbuf_t nbuf, uint8_t tid, uint8_t nodrop, uint8_t hol);
...
ol_tx_rawsim_getastentry(struct ol_txrx_vdev_t *vdev, adf_nbuf_t nbuf)
{
    u_int8_t ftype = 0;
    u_int8_t *hdr_ptr;
    ...
    ftype = adf_nbuf_get_ftype(nbuf);

    if(ftype != CB_FTYPE_MCAST2UCAST)
        eh = (struct ether_header *) nbuf->data;
    else{
        mcbuf = (struct ol_tx_me_buf_t *) adf_nbuf_get_fctx(nbuf);
    ...
    switch (vdev->opmode) {
        ...
        default:
            adf_os_print("ol_tx_classify - only AP and STA modes supported \n");
            goto error;
    }
 ...
 */
    static inline void
ol_tx_classify(struct ol_txrx_vdev_t *vdev, adf_nbuf_t nbuf, struct ol_tx_nbuf_classify *nbuf_class)
{
    ...
    int tid_q_map;
    adf_os_mem_zero(nbuf_class, sizeof(struct ol_tx_nbuf_classify));
    ...
    if (ether_type == __constant_htons(ETHERTYPE_IP))
    {
        ...
        adf_net_ipv6hdr_t *ip6h = (adf_net_ipv6hdr_t *) L3datap;
 ...
 */
uint16_t ol_tx_classify_get_peer_id(struct ol_txrx_vdev_t *vdev, adf_nbuf_t nbuf)
{
...
#define TIDQ_AVG_SZ_COMPUTE_INTERVAL 0x1FF
    if((peer->tidq[tid].avg_size_compute_cnt & TIDQ_AVG_SZ_COMPUTE_INTERVAL) == 0) {
        if(adf_nbuf_queue_len(&peer->tidq[tid].nbufq)) {
            peer->tidq[tid].msdu_avg_size =
                          (peer->tidq[tid].byte_count)/(adf_nbuf_queue_len(&peer->tidq[tid].nbufq));
        }
    }
...
static inline uint32_t
ol_tx_enqueue_pflow_ctrl(ol_txrx_vdev_handle vdev, adf_nbuf_t nbuf, uint32_t peer_id, uint8_t tid, uint8_t nodrop, uint8_t hol)
{
    ...
    current_nbufq_len = adf_nbuf_queue_len(&peer->tidq[tid].nbufq);
    /*
    ...
    if ((!nodrop && (current_nbufq_len > pdev->pflow_ctl_queue_max_len[peer_id][tid])) ||
        ...
        if (OL_CFG_RAW_TX_LIKELINESS(vdev->tx_encap_type != htt_pkt_type_raw)) {
            ...
            if (adf_os_unlikely(hol)) {
                adf_nbuf_queue_add(&peer->tidq[OL_TX_PFLOW_CTRL_PRIORITY_TID].nbufq, nbuf);
        ...
        NBUF_SET_SUBMIT_TS(nbuf, OS_GET_TIMESTAMP());
         ...
         case OL_ATH_PARAM_MIN_THRESHOLD:
            {
                if(!buff)
                {
                    ...
                    if (value > OL_TX_PFLOW_CTRL_MIN_THRESHOLD) {
                        adf_os_print("WARNING: setting the threshold value > %d \
         ...
         case OL_ATH_PARAM_CONG_CTRL_TIMER_INTV:
            {
                if(!buff) {
                    if(value != 0) {
                        ...
                        adf_os_timer_mod(&pdev->pflow_ctl_cong_timer, pdev->pflow_cong_ctrl_timer_interval);
                    } else {
                        adf_os_timer_cancel(&pdev->pflow_ctl_cong_timer);
                    }
                }
                else
         ...
         case OL_ATH_PARAM_STATS_TIMER_INTV:
            {
                if(!buff) {
                    if(value != 0) {
                        ...
                        adf_os_timer_mod(&pdev->pflow_ctl_stats_timer, pdev->pflow_ctl_stats_timer_interval);
                    } else {
                        adf_os_timer_cancel(&pdev->pflow_ctl_stats_timer);
                    }
                }
                else
         ...
         case OL_ATH_PARAM_MSDU_TTL:
            {
                if(!buff) {
                    ...
                    adf_os_print("%p: check_msdu_ttl_val %d", pdev, pdev->pflow_msdu_ttl);
                } else {
            ...
            break;
         default:
            adf_os_print("%d not handled  in %s \n", param, __func__);
            break;
    }
...
static inline uint32_t
ol_tx_check_enqueue_flow_ctrl(struct ol_txrx_pdev_t *pdev, adf_nbuf_t nbuf)
{
    if(!adf_nbuf_is_queue_empty(&pdev->acnbufq)) {
        OL_TX_FLOW_CTRL_LOCK(&pdev->acnbufqlock);
        if(adf_os_unlikely(pdev->acqcnt >= pdev->acqcnt_len)) {
            TXRX_STATS_ADD(pdev, pub.tx.fl_ctrl.fl_ctrl_discard, 1);
            OL_TX_FLOW_CTRL_UNLOCK(&pdev->acnbufqlock);
            return OL_TX_FLOW_CTRL_Q_FULL;
        } else {
            pdev->acqcnt++;
            adf_nbuf_queue_add(&pdev->acnbufq, nbuf);
            TXRX_STATS_ADD(pdev, pub.tx.fl_ctrl.fl_ctrl_enqueue, 1);
            if(adf_os_unlikely(pdev->acqcnt > (pdev->acqcnt_len - pdev->vdev_count)))
            {
                OL_TX_FLOW_CTRL_UNLOCK(&pdev->acnbufqlock);
...
static inline uint32_t
ol_tx_enqueue_flow_ctrl(struct ol_txrx_pdev_t *pdev, adf_nbuf_t nbuf)
{
    OL_TX_FLOW_CTRL_LOCK(&pdev->acnbufqlock);
    if (adf_os_unlikely(pdev->acqcnt >= pdev->acqcnt_len)) {
        TXRX_STATS_ADD(pdev, pub.tx.fl_ctrl.fl_ctrl_discard, 1);
		...
		adf_nbuf_t msdu)
{
    ...
    */
    if (adf_os_unlikely(adf_os_atomic_read(&vdev->pdev->target_tx_credit) <= 0)) {
	TXRX_STATS_MSDU_LIST_INCR(
			vdev->pdev, tx.dropped.host_reject, msdu);
#ifdef TSO_DBG
	adf_os_print("HOST_TSO::No Tx credits on Target \n");
#endif
    ...
    parent = adf_nbuf_get_parent(msdu);
    if (adf_os_unlikely(!parent)) {
	TXRX_STATS_MSDU_LIST_INCR(
			vdev->pdev, tx.dropped.host_reject, msdu);
	adf_os_print("HOST_TSO::No parent buffer \n");
	ol_tx_tso_failure_cleanup(vdev->pdev,msdu);
	return msdu; /* the list of unaccepted MSDUs */
    }

    sw_tso_desc =
	(struct ol_tx_tso_desc_t *)NBUF_EXTRA_FRAG_VADDR(parent,0);
    if (adf_os_unlikely(!sw_tso_desc)) {
	TXRX_STATS_MSDU_LIST_INCR(
			vdev->pdev, tx.dropped.host_reject, msdu);
	adf_os_print("HOST_TSO::No Tx TSO Descriptor \n");
	ol_tx_tso_failure_cleanup(vdev->pdev,msdu);
	return msdu; /* the list of unaccepted MSDUs */
    }
	...
	TXRX_STATS_MSDU_LIST_INCR(
			vdev->pdev, tx.dropped.host_reject, msdu);
#ifdef TSO_DBG
	adf_os_print("HOST_TSO::No Tx Descriptors \n");
#endif
	ol_tx_tso_failure_cleanup(vdev->pdev,msdu);
	return msdu; /* the list of unaccepted MSDUs */
    }
                           ...
                           struct ol_tx_desc_t *tx_desc)
{
    ...
    u_int8_t tidno = ADF_NBUF_TX_EXT_TID_INVALID;
#if MESH_MODE_SUPPORT
    u_int8_t power = 0;
    u_int8_t ratecode = 0;
    u_int8_t retries = 0;
   ...
   // adf_nbuf_frags_num_init(nbuf);
    ...
    if (vdev->mesh_vdev) {
        ...
        /* metahdr extraction */
        mhdr = (struct meta_hdr_s *)adf_nbuf_data(nbuf);
        power = mhdr->power;
        ...
        adf_nbuf_pull_head(nbuf, sizeof(struct meta_hdr_s));
    }
#endif
    ...
    if( ftype == CB_FTYPE_MCAST2UCAST ) {
        ...
        buf = (struct ol_tx_me_buf_t*) adf_nbuf_get_fctx(nbuf);

#if !MCAST_ENHANCE_NO_CONSISTENT_MEM
        if (buf->nonpool_based_alloc) {
            if (A_STATUS_FAILED == adf_os_mem_map_nbytes_single(pdev->osdev, &buf->buf[0], ADF_OS_DMA_TO_DEVICE, dlen,
                        &buf_paddr)) {
        ...
        if( len > dlen ) {
            ...
            for (i = 1; i < num_frags + 1; i++) {
                ...
                frag_len = adf_nbuf_get_frag_len(nbuf, i);
                frag_paddr = adf_nbuf_get_frag_paddr_lo(nbuf, i);
        ...
        sw_sg_desc = (struct ol_tx_sg_desc_t *) adf_nbuf_get_fctx(nbuf);
        ...
        sw_tso_sg_desc = (struct ol_tx_tso_desc_t *) adf_nbuf_get_fctx(nbuf);
    ...
    htt_tx_desc_init(pdev->htt_pdev, htt_tx_desc, tx_desc->id,
         len, vdev->vdev_id, htt_pkt_type_ethernet, adf_nbuf_get_tx_cksum(nbuf),
         tidno);
    ...
    if(adf_nbuf_get_tx_cksum(nbuf)==HTT_TX_L4_CKSUM_OFFLOAD) {
    ...
    if (adf_os_unlikely(len < pkt_download_len)) {
...

{
    ...
    for (i = 0; i < num_msdus; i++) {
        nbuf = nbuf_arr[i];
#if HOST_SW_TSO_ENABLE
         if(adf_os_unlikely(adf_nbuf_is_tso(nbuf) == A_TRUE)) {
...
#endif /* HOST_SW_TSO_ENABLE */
         {
             ...
             if( ftype == CB_FTYPE_MCAST2UCAST ) {
                 ...
                 if (adf_os_likely(status == OL_TX_FLOW_CTRL_ENQ_DONE) ||
                     (status == OL_TX_FLOW_CTRL_ENQ_DONE_PAUSE_THRESOLD)) {
                     adf_nbuf_set_vdev_ctx(nbuf, (a_int32_t)vdev);
                     if(adf_os_unlikely(status == OL_TX_FLOW_CTRL_ENQ_DONE_PAUSE_THRESOLD)) {
...
#if HOST_SW_TSO_SG_ENABLE
                    if (ftype == CB_FTYPE_TSO_SG )
                        ol_tx_tso_sg_desc_free(pdev, (struct ol_tx_tso_desc_t *)adf_nbuf_get_fctx(nbuf));
#endif
#if HOST_SW_SG_ENABLE
                    if (ftype == CB_FTYPE_SG)
                        ol_tx_sg_desc_free(pdev, (struct ol_tx_sg_desc_t *)adf_nbuf_get_fctx(nbuf));
#endif
...
static void ol_vap_send_overflow_event(ol_txrx_vdev_handle vdev)
{
    osif_dev * osdev = (osif_dev *)(vdev->osif_vdev);
    struct ieee80211vap *vap = osdev->os_if;
    a_uint64_t tick_current = adf_os_ticks();
#if !PEER_FLOW_CONTROL_FORCED_MODE0
    static u_int32_t txoverflow_limit_print = 0;
#endif

    if(adf_os_time_after(tick_current,vdev->tx_overflow_start+HZ)){
...
ol_tx_ll_cachedhdr_prep(ol_txrx_vdev_handle vdev,
        adf_nbuf_t netbuf, uint32_t peerid, uint8_t tid, struct ol_tx_desc_t *tx_desc )
{
    struct ol_txrx_pdev_t *pdev = vdev->pdev;
    struct ol_txrx_peer_t *peer;
    u_int8_t *data;
    ...
    if (vdev->mesh_vdev) {
        ...
        kix_valid_flags |= ((mhdr->keyix & 0x3) << 6);
        adf_nbuf_pull_head(netbuf, sizeof(struct meta_hdr_s));
    ...
    if (OL_CFG_RAW_TX_LIKELINESS(vdev->tx_encap_type == htt_pkt_type_raw)) {
        if (adf_os_unlikely(OL_TX_PREPARE_RAW_DESC_CHDHDR(pdev,
                        vdev, netbuf, tx_desc,
    ...
    data = adf_nbuf_push_head(netbuf, vdev->htc_htt_hdr_size);
    adf_os_mem_copy(data, hdr_cache, vdev->htc_htt_hdr_size);

    adf_nbuf_peek_header(netbuf, (a_uint8_t **)&netdata, &netlen);
        ...
        if (OL_CFG_RAW_TX_LIKELINESS(vdev->tx_encap_type == htt_pkt_type_raw)) {
            ...
            while (currnbuf) {
                ...
                adf_nbuf_unmap_single(
                    vdev->osdev, currnbuf, ADF_OS_DMA_TO_DEVICE);
...
#if TCP_CHECKSUM_OFFLOAD
    if( adf_os_unlikely(adf_nbuf_get_tx_cksum(netbuf)==HTT_TX_L4_CKSUM_OFFLOAD) ) {
        uint32_t *cksum_enable;
        cksum_enable = (uint32_t *) (((uint32_t *)tx_desc->htt_frag_desc) + 3);
        *cksum_enable |= HTT_ENDIAN_BYTE_IDX_SWAP(0x1f0000);
    }
...
ol_tx_ll_pflow_ctrl(ol_txrx_vdev_handle vdev, adf_nbuf_t netbuf)
{
    ...
    ftype = adf_nbuf_get_ftype(netbuf);
    if ( ( ftype != CB_FTYPE_MCAST2UCAST)
#if HOST_SW_TSO_SG_ENABLE
...
#define OL_NAWDS_ENABLED(_vdev) (_vdev->nawds_enabled & 0x1)
    ...
    if (adf_os_unlikely(OL_NAWDS_ENABLED(vdev)) && nbuf_class.is_mcast) {
    ...
    adf_nbuf_set_vdev_ctx(netbuf, (a_int32_t)vdev);
    status = ol_tx_enqueue_pflow_ctrl(vdev, netbuf, nbuf_class.peer_id, nbuf_class.tid , nodrop, hol);

    if (adf_os_likely(status == OL_TX_PFLOW_CTRL_DESC_AVAIL)) {
        return OL_TXRX_SUCCESS;
    } else if (adf_os_likely(status == OL_TX_PFLOW_CTRL_ENQ_DONE)) {
        return OL_TXRX_SUCCESS;
    } else {
                                                                      ...
                                                                      uint32_t peer_id, uint8_t tid)
{
    ...
    adf_nbuf_t tnetbuf = NULL;
    adf_nbuf_t hnetbuf = NULL;
    ol_txrx_vdev_handle vdev;

    while(!adf_nbuf_is_queue_empty(&peer->tidq[OL_TX_PFLOW_CTRL_PRIORITY_TID].nbufq)) {
        /* Take first skb frame from tidq */
        netbuf = adf_nbuf_queue_first(&peer->tidq[OL_TX_PFLOW_CTRL_PRIORITY_TID].nbufq);
        if(netbuf) {
            tx_desc = ol_tx_desc_alloc(pdev);
            if(tx_desc) {
                /* Pop netbuf from tidq */
                netbuf = adf_nbuf_queue_remove(&peer->tidq[OL_TX_PFLOW_CTRL_PRIORITY_TID].nbufq);

                vdev = (ol_txrx_vdev_handle )adf_nbuf_get_vdev_ctx(netbuf);
                peer->tidq[tid].byte_count -= adf_nbuf_len(netbuf);
                ...
                if (OL_CFG_RAW_TX_LIKELINESS(vdev->tx_encap_type != htt_pkt_type_raw)) {
                    ...
                    if (tnetbuf == NULL) {
                        ...
                        adf_nbuf_set_next(tnetbuf, netbuf);
                        tnetbuf = netbuf;
                    }
                }
            } else { /* !tx_desc */
            ...
            if (!adf_nbuf_is_queue_empty(&peer->tidq[OL_TX_PFLOW_CTRL_PRIORITY_TID].nbufq))  {
                adf_os_print("%s queue not empty %d , but nbuf NULL \n", __func__, adf_nbuf_queue_len(&peer->tidq[OL_TX_PFLOW_CTRL_PRIORITY_TID].nbufq));
            }
...
out:
    ol_tx_pflow_ctrl_queue_map_update(pdev, peer, peer_id, tid);
    if(hnetbuf) {
        CE_send_list(pdev->ce_tx_hdl, hnetbuf, vdev->epid, vdev->downloadlen, 0);
    }
...
ol_tx_pflow_ctrl_dequeue_send(struct ol_txrx_pdev_t *pdev, uint32_t *fetch_ind_rec, uint32_t *num_tx_pkts, uint32_t *num_tx_bytes)
{
    ...
    adf_os_print("%s peer_id = %d tid = %d max_pkts = %d max_bytes = %d \n", __func__, peer_id, tid, max_pkts, max_bytes);
#endif
    ...
    while (tx_pkts <= max_pkts) {
        ...
        if (adf_nbuf_is_queue_empty(&peer->tidq[tid].nbufq)) {
            OL_TX_PEER_UNLOCK(pdev, peer_id);
            status = OL_TX_PFLOW_CTRL_DEQ_REMAINING;
            goto out;
        }

        netbuf = adf_nbuf_queue_first(&peer->tidq[tid].nbufq);

        if (netbuf) {
            vdev = (ol_txrx_vdev_handle )adf_nbuf_get_vdev_ctx(netbuf);
            adf_os_assert_always(vdev);
            adf_os_assert_always(vdev->pdev);
...
#if HOST_SW_TSO_SG_ENABLE
                if(ftype == CB_FTYPE_TSO_SG) {
                    num_bytes = ol_tx_tso_sg_get_nbuf_len(netbuf);
                } else
#endif
                {
                    num_bytes = adf_nbuf_len(netbuf);
                }
                tx_bytes += num_bytes;
            } else {
            ...
            if (tx_desc) {
                if (OL_CFG_RAW_TX_LIKELINESS(vdev->tx_encap_type != htt_pkt_type_raw)) {
                    netbuf = adf_nbuf_queue_remove(&peer->tidq[tid].nbufq);
#if HOST_SW_TSO_SG_ENABLE
                    if(ftype == CB_FTYPE_TSO_SG) {
                ...
                if ( ( ftype == CB_FTYPE_MCAST2UCAST)
                    ...
                    if(scn->enable_perpkt_txstats) {
                        NBUF_SET_SUBMIT_TS(netbuf, OS_GET_TIMESTAMP());
                    }
#endif

                    if ((status == OL_TXRX_SUCCESS) &&
                            (1 == CE_send_fast(pdev->ce_tx_hdl, &netbuf, 1, ep_id))) {
            ...
            if (!adf_nbuf_is_queue_empty(&peer->tidq[tid].nbufq))  {
                adf_os_print("%s queue not empty %d , but nbuf NULL \n", __func__, adf_nbuf_queue_len(&peer->tidq[tid].nbufq));
            } else {
                QUEUE_MAP_CNT_RESET(pdev, peer_id, tid);
            }
...
u_int32_t
ol_tx_ll_fetch_sched(struct ol_txrx_pdev_t *pdev, uint32_t num_records, uint32_t *fetch_ind_record, adf_nbuf_t fetch_resp_buf)
{
    int i;
    u_int32_t status;
    ...
    while(i < num_records) {
        ...
        fetch_resp_record = (u_int32_t *) adf_nbuf_put_tail(fetch_resp_buf,8);
        if (fetch_resp_record == NULL) {
            adf_os_print("%s: Failed to expand head\n", __func__);
            goto out;
        }
...
ol_tx_ll_sched(struct ol_txrx_pdev_t *pdev)
{
    ...
    do {
        ...
        if (pdev->acqcnt) {
            tx_desc = ol_tx_desc_alloc(pdev);
            if (tx_desc) {
                netbuf = adf_nbuf_queue_remove(&pdev->acnbufq);
                pdev->acqcnt--;
            } else {
                TXRX_STATS_ADD(pdev, pub.tx.desc_alloc_fails, 1);
                OL_TX_FLOW_CTRL_UNLOCK(&pdev->acnbufqlock);
...
void  ol_ll_pdev_tx_lock(void* ptr){
    ...
    struct ol_txrx_pdev_t *pdev = vdev->pdev;
    adf_os_spin_lock_bh(&pdev->tx_lock);
...
void  ol_ll_pdev_tx_unlock(void* ptr){
    ...
    struct ol_txrx_pdev_t *pdev = vdev->pdev;
    adf_os_spin_unlock_bh(&pdev->tx_lock);
...
#if PEER_FLOW_CONTROL
		adf_nbuf_t netbuf ,uint16_t peer_id, uint8_t tid)
#else
...
#endif
{
    ...
    adf_nbuf_set_fctx_type(netbuf, 0, 0);

    if (adf_os_unlikely((headroom = adf_nbuf_headroom(netbuf))< vdev->htc_htt_hdr_size)){
        netbuf = adf_nbuf_realloc_headroom(netbuf, vdev->htc_htt_hdr_size);
        if (netbuf == NULL ){
            adf_os_print("Realloc failed \n");
            goto out;
        }
...
#undef ARP_CMD_OFFSET
        ...
        if (adf_os_ntohs(eh->h_proto) == ETHERTYPE_ARP) {
    ...
    adf_nbuf_t msdu_list)
{
    ...
    if(tx_spec & ol_txrx_osif_tx_spec_tso) {
	    ...
	    if (adf_os_unlikely(!msdu))  return NULL;
    }
#endif /* HOST_SW_TSO_ENABLE */
    ...
    while (msdu) {
	    ...
	    if( ol_tx_tso_prepare_ll(&tx_desc, vdev, msdu) )
		    return msdu;
	    if(adf_os_unlikely(tx_desc == NULL))
		    return msdu;
    } else
#endif /* HOST_SW_TSO_ENABLE */
...
ol_tx_flow_ctrl_tidq_stats(struct ol_txrx_pdev_t *pdev, u_int32_t peer_id, u_int8_t tid)
{
    ...
    peer = ol_txrx_peer_find_by_id(pdev, peer_id);
    if (!peer) {
        printk("%s peer=NULL %d\n", __func__, peer_id);
        return;
    }
    adf_os_print("----------------------\n");
    adf_os_print(" peer_id %d, tid %d\n", peer_id, tid);
    adf_os_print("---------------+------\n");
    /*
    ...
    */
    ...
    adf_os_print("encoded_byte_cnt   | 0x%x\n", QUEUE_MAP_BYTE_CNT_GET(pdev,peer_id,tid));
#else
    adf_os_print("encoded_byte_cnt   | 0x%x\n", pdev->pmap->byte_cnt[tid][peer_id]);
#endif
    adf_os_print("bytes         | %u\n", peer->tidq[tid].byte_count);
    adf_os_print("queue_len     | %d\n", adf_nbuf_queue_len(&peer->tidq[tid].nbufq));
    adf_os_print("max_queue_len | %d\n", pdev->pflow_ctl_queue_max_len[peer_id][tid]);
    adf_os_print("high_watermark| %u\n", peer->tidq[tid].high_watermark);
...
ol_tx_pflow_ctrl_stats_timer(void *arg)
{
    ...
    adf_os_print("------- per-Peer/TIDQ Stats ----------------\n");
    tid = 0;
...
void ol_txrx_per_peer_stats(struct ol_txrx_pdev_t *pdev, char *addr)
{
    ...
    if (status != 0) {
        adf_os_print("NSS command send failed for peer stats \n");
        ieee80211_free_node(ni);
        return;
    }
#endif

    adf_os_print("------- Assoc ID = %d ----------------\n", IEEE80211_AID(ni->ni_associd));
    ieee80211_free_node(ni);
...
ol_tx_pflow_ctrl_ttl(struct ol_txrx_pdev_t *pdev, u_int32_t peer_id, u_int8_t tid)
{
    ...
    qdepth = adf_nbuf_queue_len(&peer->tidq[tid].nbufq);
...
ol_tx_pflow_enqueue_nawds(void *arg, adf_nbuf_t nbuf, uint8_t tid, uint8_t nodrop, uint8_t hol)
{
    ...
    TAILQ_FOREACH(peer, &vdev->peer_list, peer_list_elem) {
        if(peer && peer->peer_ids[0] != HTT_INVALID_PEER &&  (peer->bss_peer || peer->nawds_enabled)) {
            netbuf_copy = adf_nbuf_copy(nbuf);
            if (netbuf_copy) {
                adf_nbuf_reset_ctxt(netbuf_copy);
                adf_nbuf_set_vdev_ctx(netbuf_copy, (a_int32_t)vdev);
...
ol_tx_flush_tid_queue_pflow_ctrl(struct ol_txrx_pdev_t *pdev, u_int16_t peer_id, u_int8_t tid)
{
    ...
    while(!adf_nbuf_is_queue_empty(&peer->tidq[tid].nbufq)) {
        netbuf = adf_nbuf_queue_remove(&peer->tidq[tid].nbufq);
        if (netbuf)
            ol_tx_free_buf_generic(pdev, netbuf);
        pdev->pflow_ctl_global_queue_cnt--;
...
ol_tx_switch_mode_flush_buffers(struct ol_txrx_pdev_t *pdev)
{
    ...
    while (tid < OL_TX_PFLOW_CTRL_HOST_MAX_TIDS) {
        ...
        while(peer_id < OL_TXRX_MAX_PEER_IDS) {
            ...
            if(peer) {
                if(tid != OL_TX_PFLOW_CTRL_PRIORITY_TID) {
                    ol_tx_ll_pflow_sched(pdev, peer_id, tid, adf_nbuf_queue_len(&peer->tidq[tid].nbufq));
                }
                /* Drop pkts */
                if(adf_nbuf_queue_len(&peer->tidq[tid].nbufq))
                    ol_tx_flush_tid_queue_pflow_ctrl(pdev, peer_id, tid);
