...
int ol_ath_ucfg_setparam(struct ol_ath_softc_net80211 *scn, int param, int value)
{
    ...
    if (scn->down_complete) {
        adf_os_print("Starting the target before sending the command\n");
        if (ol_ath_target_start(scn)) {
            adf_os_print("failed to start the target\n");
            return -1;
        }
    }
        ...
        switch (param) {
            case OL_SPECIAL_PARAM_COUNTRY_ID:
                ...
                if (retval) {
                    ...
                    adf_os_print("%s: Unable to set country code \n",__func__);
...
int ol_ath_ucfg_get_user_postion(wlan_if_t vaphandle, u_int32_t value)
{
    ...
    if (!vaphandle) {
        adf_os_print("get_user_pos:vap not available\n");
        return A_ERROR;
    }
    ...
    if (ol_ath_ucfg_validate_aid(vaphandle, value)){
        buf = wmi_buf_alloc(scn->wmi_handle, sizeof(wmi_peer_gid_userpos_list_cmd));
        if (buf == NULL) {
           return A_NO_MEMORY;
        }
    wbuf_append(buf, sizeof(wmi_peer_gid_userpos_list_cmd));
    cmd = (wmi_peer_gid_userpos_list_cmd *)(wbuf_header(buf));
    OS_MEMCPY(&cmd->aid, &value, sizeof(value));

    status = wmi_unified_cmd_send(scn->wmi_handle, buf,
...
int ol_ath_ucfg_reset_peer_mumimo_tx_count(wlan_if_t vaphandle, u_int32_t value)
{
    ...
    if (!vaphandle) {
        adf_os_print("reset_peer_mumimo_tx_count:vap not available\n");
        return A_ERROR;
    }
    ...
    scn = OL_ATH_SOFTC_NET80211(ic);
    if (ol_ath_ucfg_validate_aid(vaphandle, value)){
        buf = wmi_buf_alloc(scn->wmi_handle, sizeof(wmi_peer_txmu_rstcnt_cmd));
        if (buf == NULL) {
           return A_NO_MEMORY;
        }
    wbuf_append(buf, sizeof(wmi_peer_txmu_rstcnt_cmd));
    cmd = (wmi_peer_txmu_rstcnt_cmd *)(wbuf_header(buf));
    OS_MEMCPY(&cmd->aid, &value, sizeof(value));
...
int ol_ath_ucfg_get_peer_mumimo_tx_count(wlan_if_t vaphandle, u_int32_t value)
{
    ...
    if (!vaphandle) {
        adf_os_print("mumimo_tx_count:vap not available\n");
        return A_ERROR;
    }
    ...
    scn = OL_ATH_SOFTC_NET80211(ic);
    if (ol_ath_ucfg_validate_aid(vaphandle, value)){
        buf = wmi_buf_alloc(scn->wmi_handle, sizeof(wmi_peer_txmu_cnt_cmd));
        if (buf == NULL) {
           return A_NO_MEMORY;
        }
    wbuf_append(buf, sizeof(wmi_peer_txmu_cnt_cmd));
    cmd = (wmi_peer_txmu_cnt_cmd *)(wbuf_header(buf));
    OS_MEMCPY(&cmd->aid, &value, sizeof(value));
        ...
        int intrmode)
{
    ...
    return wmi_unified_gpio_config(scn->wmi_handle, gpionum, input, pulltype, intrmode);
}
...
int ol_ath_ucfg_gpio_output(struct ol_ath_softc_net80211 *scn, int gpionum, int set)
{
    ...
    return wmi_unified_gpio_output(scn->wmi_handle, gpionum, set);
}
...
int ol_ath_ucfg_get_ath_stats(struct ol_ath_softc_net80211 *scn, struct ath_stats_container *asc)
{
    ...
    if(asc->size == 0 || asc->address == NULL) {
        ...
        if(asc->flag_ext & EXT_TXRX_FW_STATS) {  /* fw stats */
            ...
            if (ol_txrx_fw_stats_get(vap->iv_txrx_handle, &req, PER_RADIO_FW_STATS_REQUEST) != 0) {
...
int ol_ath_ucfg_get_packet_power_info(struct ol_ath_softc_net80211 *scn,
        ...
        u_int16_t hw_rate)
{
    if(wmi_unified_packet_power_info_get(scn->wmi_handle, rate_flags, nss, preamble, hw_rate)) {
