...
extern void ath_lowi_if_nlsend_response(struct ieee80211com *ic, u_int8_t *data, u_int16_t datalen, u_int8_t msgtype, u_int32_t msg_subtype);

static void ol_ath_cache_lci(void *req);
                    ...
                    u_int16_t datalen, void *context)
{
    ...
    ath_lowi_if_nlsend_response(ic, data, datalen, ANI_MSG_OEM_DATA_RSP, TARGET_OEM_CAPABILITY_RSP);
    return 0;
}
                    ...
                    u_int16_t datalen, void *context)
{
    ...
    ath_lowi_if_nlsend_response(ic, data, datalen, ANI_MSG_OEM_DATA_RSP, TARGET_OEM_MEASUREMENT_RSP);
    return 0;
}
                                  ...
                                  int msg_len, void *req, int msgsubType)
{
    ...
    if (msgsubType == TARGET_OEM_CONFIGURE_LCI) {
        ol_ath_cache_lci(req+4); //move msgsubType (4 octets)
    ...
    OL_IF_MSG_COPY_CHAR_ARRAY(head, req, msg_len);

    if(wmi_unified_cmd_send(scn->wmi_handle, buf, msg_len, WMI_OEM_REQ_CMDID)) {
        adf_os_print("%s: ERROR: Host unable to send LOWI request to FW \n", __func__);
        return -1;
    }
...
static void ol_ath_cache_lcr(void *req)
{
    ...
    num_ap_lcr = 1;
    adf_os_print("Copied LCR data to driver cache (%d octets)\n", loc_civic_len);
}
...
static void ol_ath_cache_lci(void *req)
{
    ...
    /* Save LCI data in host buffer */
    {
        ...
        num_ap_lci = 1;
        adf_os_print("Copied LCI data to driver cache\n");
    }
}
