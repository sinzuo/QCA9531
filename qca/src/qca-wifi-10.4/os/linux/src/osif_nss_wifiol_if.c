...
void osif_nss_vdev_aggregate_msdu(ar_handle_t arh, struct ol_txrx_pdev_t *pdev, adf_nbuf_t msduorig, adf_nbuf_t *head_msdu, uint8_t no_clone_reqd);
        ...
        case NSS_WIFI_INIT_MSG:
            {
                ...
                printk("NSS WIFI init succeded for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
        ...
        case NSS_WIFI_TX_INIT_MSG:
            {
                ...
                printk("NSS WIFI TX init succeded for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
...
ol_rx_process_inv_peer(ol_txrx_pdev_handle pdev, void *rx_mpdu_desc,
		adf_nbuf_t msdu);
void
phy_data_type_chk(adf_nbuf_t head_msdu, u_int32_t *cv_chk_status);

void
cbf_type_chk(adf_nbuf_t head_msdu, u_int32_t *cv_chk_status);

/*
 * To check whether msdu is rx_cbf_type
 */
int osif_nss_ol_is_rx_cbf(adf_nbuf_t msdu, struct htt_pdev_t *htt_pdev)
{
...
static void osif_nss_ol_wifi_ext_callback_func(void * ctx , struct sk_buff * nbuf,  __attribute__((unused)) struct napi_struct *napi)
{
    ...
    struct ol_txrx_pdev_t *pdev = htt_pdev->txrx_pdev;
    adf_nbuf_t mon_skb, skb_list_head, next;
    int discard = 1;
        ...
        default :
            {
                ...
                if ((pdev->monitor_vdev) && (!pdev->filter_neighbour_peers)) {
                    ...
                    if (skb_list_head) {
                        ...
                        if (adf_os_likely((not_mgmt_type && not_ctrl_type) != 0)) {

                            is_mcast = IEEE80211_IS_MULTICAST(eh->ether_dhost);

                            if (adf_os_likely(is_mcast != 1)) {
                                /*ucast data pkts*/
                                if (ic->mon_filter_ucast_data == 1) {
                                    /*drop ucast pkts*/
                                    drop_packet = 1;
                                }
                            } else {
                                /*mcast data pkts*/
                                if (ic->mon_filter_mcast_data == 1) {
                                    /*drop mcast pkts*/
                                    drop_packet = 1;
                                }
                            }
...
int osif_nss_ol_wifi_reset(void *hifctx, uint16_t delay)
{
    ...
    sc->scn->nss_wifiol_ce_enabled = 0;
