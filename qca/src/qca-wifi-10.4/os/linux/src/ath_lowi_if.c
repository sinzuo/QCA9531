...
void ath_lowi_if_netlink_send(void *buf, int len)
{
    ...
    if (!skb) {
        adf_os_print("%s: No memory for skb\n", __func__);
        return;
    }

    nlh = nlmsg_put(skb, pid, 0, 0, len, 0);
    if (!nlh) {
        adf_os_print("%s: nlmsg_put() failed\n", __func__);
        kfree_skb(skb);
        return;
    }
...
{
    if (!lowi_if_nl) {
        adf_os_print("%s lowi_if_nl is NULL\n", __func__);
        return -ENODEV;
    }
    adf_os_print("%s Going to decrement current LOWI netlink ref count: %d\n", __func__, atomic_read(&lowi_if_nl->lowi_if_refcnt));
...
void ath_lowi_if_processmsg(void *data)
{
    ...
    if(!ic){
        adf_os_print("%s: ERROR: No offload-capable radio with active VAP was found. Dropping LOWI message\n", __func__);
        return;
    }
    ...
    switch(ani_hdr->msg_type)
    {
        ...
        case ANI_MSG_OEM_DATA_REQ:
            ...
            switch(msgsubType) {
                ...
                case TARGET_OEM_MEASUREMENT_REQ:
                    ...
                    adf_os_print("%s: Received RTT measurement request from LOWI \n", __func__);
                    /* Find radio that supports this channel */
                    ic = ath_lowi_if_get_ic(NULL, &wmi_ch.mhz, NULL);
                    if (!ic) {
                            adf_os_print("%s: ERROR: No Radio Found supporting channel (%d mhz). Dropping ranging request.\n", __func__, wmi_ch.mhz);
                        return;
                    }
                    /* Send message to FW */
                    if(ic->ic_lowi_frame_send)
                ...
                case TARGET_OEM_CONFIGURE_LCI:
                    adf_os_print("%s: Received LCI/LCR request from LOWI \n", __func__);
                ...
                case TARGET_OEM_CONFIGURE_WRU:
                    adf_os_print("%s: Received Where Are You frame request from LOWI \n", __func__);
                ...
                case TARGET_OEM_CONFIGURE_FTMRR:
                    adf_os_print("%s: Received FTMRR frame request from LOWI \n", __func__);
                ...
                default:
                    adf_os_print("%s: ERROR: UNKNOWN message sub-type received for ANI_MSG_OEM_DATA_REQ lowi message\n", __func__);
                    return;
            }
        ...
        default:
            adf_os_print("%s: ERROR: UNKNOWN LOWI message received\n", __func__);
    }
}
...
void ath_lowi_if_nlsend_reg_resp(struct ieee80211com *ic)
{
    ...
    adf_os_print("LOWI is now up, registration completed\n");
...
void ath_lowi_if_nlsend_response(struct ieee80211com *ic,
        u_int8_t *data, u_int16_t datalen, u_int8_t msgtype, u_int32_t msg_subtype)
{
    ...
    {
        err_code = (u_int32_t *) ((char*) data + sizeof (wmi_rtt_event_hdr));
        adf_os_print("%s: Error report recevied from FW: Error Code: %d\n", __func__, *err_code);
    }
    /* Allocate buffer for entire message including header */
