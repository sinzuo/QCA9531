...
void populate_tx_info(struct ol_ath_softc_net80211 *scn, adf_nbuf_t  netbuf, int i, int num_msdus, enum htt_tx_status status, struct ieee80211_tx_status  *ts, uint8_t mhdr_len, bool is_raw);
...
void osif_nss_vdev_aggregate_msdu(ar_handle_t arh, struct ol_txrx_pdev_t *pdev, adf_nbuf_t msduorig, adf_nbuf_t *head_msdu, uint8_t no_clone_reqd)
{
    ...
    int is_head = 0;
    int is_tail = 0;
    adf_nbuf_t nbuf, next, msdu = NULL;
    ...
    *head_msdu = NULL;
     ...
     */
    adf_nbuf_set_next(pdev->mon_mpdu_skb_list_tail, msdu);
    pdev->mon_mpdu_skb_list_tail = msdu;
    adf_nbuf_set_next(msdu, NULL);
...
static bool osif_nss_vdev_handle_monitor_mode(struct net_device *netdev, struct sk_buff *skb, uint8_t is_chain)
{
    ...
    if (likely((not_mgmt_type && not_ctrl_type) != 0)) {
        if ((is_mcast && (ic->mon_filter_mcast_data == 1)) || ((!is_mcast) && (ic->mon_filter_ucast_data == 1))) {
...
osif_nss_vdev_data_receive(struct net_device *dev, struct sk_buff *skb, __attribute__((unused)) struct napi_struct *napi)
{
    ...
    if ((vdev->opmode == wlan_op_mode_monitor) || vap->iv_special_vap_mode || vap->iv_smart_monitor_vap) {
        if(is_chain) {
            ...
            while (skb_head) {
                skb_next = adf_nbuf_next(skb_head);
                adf_nbuf_free(skb_head);
                skb_head = skb_next;
            }
        } else {
            adf_nbuf_free(skb);
