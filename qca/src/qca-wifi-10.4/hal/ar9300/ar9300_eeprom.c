...
ar9300_calibration_data_read_file(struct ath_hal *ah, u_int8_t *buffer, int many)
{
    ...
    ret_val = adf_os_fs_read(filename, 0, (unsigned int)many, buffer);
    ...
    int mdata_size)
{
    ...
    /* AH_CAL_IN_FILE_HOST is used to define load calibration data from file
       if caliration data is in file, skip restore from DRAM */
#ifdef AH_CAL_IN_FILE_HOST
    if(ahp->try_file)
        return -1;
#endif
