    ...
    osdev_t                     st_osdev)
{
    ...
    scan_entry = (struct ieee80211_scan_entry *)adf_mempool_alloc(vaphandle->iv_ic->ic_adf_dev, vaphandle->iv_ic->mempool_net80211_scan_entry);
    if (scan_entry == NULL) {
    ...
    if (vaphandle->iv_mesh_vap_mode) {
        ...
        TAILQ_FOREACH_SAFE(scan_entry_match, &(scan_table->st_entry), se_list, next) {
            if( (scan_entry_match->se_flag & IEEE80211_SE_FLAG_IS_MESH) &&
                (long unsigned int)ieee80211_scan_entry_age(scan_entry_match) > (1000 * ic->ic_scan_entry_timeout) ) {
...
{
    if (adf_mempool_init(ic->ic_adf_dev, &ic->mempool_net80211_scan_entry,
                        ...
                        struct ieee80211_scan_entry *scan_entry)
{
    ...
    if (atomic_read(&scan_entry->se_refcount) == 0) {
        ...
        IEEE80211_DUMP_TRACESE(scan_entry);
        adf_mempool_free(st_ic->ic_adf_dev, st_ic->mempool_net80211_scan_entry, scan_entry);
    }
    else {
...
bool scanner_check_for_bssid_entry(void  *arg)
{
    ...
    if (vaphandle == NULL) {
        adf_os_print("%s: vaphandle is NULL \n", __func__);
        return false;
    }
    ...
    if (desired_bssid) {
        ...
        TAILQ_FOREACH_SAFE(scan_entry_tmp, &(scan_table->st_entry), se_list, next) {
            if (adf_os_mem_cmp(scan_entry_tmp->se_bssid,des_bssid,IEEE80211_ADDR_LEN) == 0 ) {
                adf_os_print("%d comapre des_bssid  %s  ",__LINE__,ether_sprintf(des_bssid) );
                adf_os_print("siid : %s  BSSID :%s \n",&scan_entry_tmp->se_ssid[2],ether_sprintf(scan_entry_tmp->se_bssid));
                match_found = 1;
                break;
            }
        }
