...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
            osif_nss_ol_vdev_set_cfg(vap->iv_txrx_handle, OSIF_NSS_VDEV_ENABLE_ME);
#endif
	}
        break;
        ...
        switch(val)
        {
            case 0:
                 if (vap->mu_cap_war != 0)
                 {
                     ...
                     /*cancel timer if it is running state*/
                     if (vap->iv_mu_timer_state == MU_TIMER_RUNNING)
                     {
                          if (vap->mu_cap_post_war == 0)
                              vap->mu_cap_post_war = 1;
                     }else{
                          if ((vap->mu_cap_client_num == 0) && ( vap->mu_cap_dedicate_client_num != 0))
                          {
                              vap->iv_mu_cap_state_switch = MU_CAP_STATE_CHG;
                              adf_os_spin_lock_bh(&vap->iv_mu_cap_lock);
                              vap->iv_mu_timer_state = MU_TIMER_RUNNING;
                              OS_SET_TIMER(&vap->iv_mu_cap_timer,10*1000); /* enable timer in 10 seconds*/
                    ...
                    "%s : MU CAP WAR has been disabled, do not need to re-disable it again\n",__func__);
                 }
                 break;
            case 1:
                 if (vap->mu_cap_war == 0)
                 {
                     vap->mu_cap_war = val;
                     /* Enable timer if confition is fit*/
                     if ((vap->mu_cap_client_num == 0) && ( vap->mu_cap_dedicate_client_num != 0))
                     {
                        vap->iv_mu_cap_state_switch = MU_CAP_STATE_CHG;
                        adf_os_spin_lock_bh(&vap->iv_mu_cap_lock);
                    ...
                    IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_DEBUG,
                    "%s : MU CAP WAR has been enabled, do not need to re-enable it again\n",__func__);
                 }
                 break;
            default:
                 IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_DEBUG,
                 "%s : MU CAP WAR enable/disable set 1/0, other value will be ignored\n",__func__);
                 break;
        }
        break;
    case IEEE80211_CONFIG_MU_CAP_TIMER:
        vap->mu_cap_timer_period = val;
        break;
#if UMAC_SUPPORT_ACL
    ...
    case IEEE80211_BEACON_RATE_FOR_VAP:
        {
            ...
            if(value == false){
                adf_os_print("%s : This rate is not allowed. Please try a valid rate.\n",__func__);
                return -EINVAL;
            }
        }
        break;
    ...
    case IEEE80211_CONFIG_DISABLE_SELECTIVE_LEGACY_RATE:
        {
            ...
            */
            if (vap->iv_disabled_legacy_rate_set == (val & 0xFFF)) {
                adf_os_print("%s : This basic rate disable mask is already configured. Please try a different value.\n",__func__);
                return 0;
            }
...
#if ATH_PERF_PWR_OFFLOAD
    adf_os_print("Configuring cts2self protection for DTIM beacon : %s \n",enable?"enable":"disable");
    /* Update in F/W as well. */
    if(ic->ic_is_mode_offload(ic))
            ol_ath_set_vap_cts2self_prot_dtim_bcn(vap);
#else
    adf_os_print("Configuring cts2self protection for DTIM beacon not supported on this platform \n");
#endif
    return 0;
...
int wlan_set_vap_priority_dscp_tid_map(wlan_if_t  vap, u_int8_t priority)
{
    ...
    {
        adf_os_print("%s: Vap Priority should be chosen in between 0 and 3\n",__func__);
        return -1;
    }
    tid = priority_to_tid[priority];
    while(count < WMI_DSCP_MAP_MAX) {
...
void update_dscp_tid_map_to_fw(wlan_if_t  vap) {
    struct ieee80211com *ic = vap->iv_ic;
...
int wlan_set_wmm_param(wlan_if_t vaphandle, wlan_wme_param param, u_int8_t isbss, u_int8_t ac, u_int32_t val)
{
    ...
    case WLAN_WME_CWMIN:
        if (isbss)
        {
            ...
            ic->bssPhyParamForAC[ac][mode].logcwmin = val;
        }
        else
        {
    ...
    case WLAN_WME_CWMAX:
        if (isbss)
        {
            ...
            ic->bssPhyParamForAC[ac][mode].logcwmax = val;
        }
        else
        {
    ...
    case WLAN_WME_AIFS:
        if (isbss)
        {
            ...
            ic->bssPhyParamForAC[ac][mode].aifsn = val;
        }
        else
        {
    ...
    case WLAN_WME_TXOPLIMIT:
        if (isbss)
        {
            ...
            ic->bssPhyParamForAC[ac][mode].txopLimit = IEEE80211_US_TO_TXOP(val);
        }
        else
        {
            ...
            wme->wme_chanParams.cap_wmeParams[ac].wmep_txopLimit
                = IEEE80211_US_TO_TXOP(val);
            ic->phyParamForAC[ac][mode].txopLimit = IEEE80211_US_TO_TXOP(val);
        }
        break;
    case WLAN_WME_ACM:
        if (!isbss)
        ...
        ic->bssPhyParamForAC[ac][mode].acm = val;
        break;
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    osif_nss_ol_wifi_monitor_set_filter(ic, filter_type);
#endif
