...
int wlan_mlme_join_infra(wlan_if_t vaphandle, wlan_scan_entry_t bss_entry, u_int32_t timeout)
{
    ...
    if (error == EOK) {
        ...
        if(mlme_is_stacac_needed(vap)) {
            adf_os_print("STACAC_start chan %d timeout %d sec, curr time: %d sec\n",
        ...
        struct ieee80211_channel *prevchan)
{
    ...
    for (i = 0; i < cur_dfs_subchan_list.cl_nchans; i++) {
        ...
        if (is_found == false) {
            adf_os_print("%s : RootAP sub-channels is not a subset of Repeater AP sub-channels\n", __func__);
            break;
        }
    }

    /* If is_found == 0, then Repeater AP has to do CAC */
    adf_os_print("%s : is_found = %d\n", __func__, is_found);
    return is_found;
}
