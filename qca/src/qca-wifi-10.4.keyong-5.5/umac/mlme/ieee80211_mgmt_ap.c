...
int ieee80211_vht_nss_mcs_valid(struct ieee80211_node *ni, u_int8_t *vcap)
{
    ...
    int rx_vhtrates = adf_os_le16_to_cpu(vhtcap->rx_mcs_map);
    struct ieee80211vap *vap = ni->ni_vap;
...
ieee80211_recv_asreq(struct ieee80211_node *ni, wbuf_t wbuf, int subtype)
{
    ...
    u_int8_t *dedicated_client_fg = NULL;
...
#define ADD_NI_RATES_NUM 8
    ...
    if ((IEEE80211_IS_CHAN_11AC(ic->ic_curchan) ||
		...
		adf_os_print("****** FW configured HT rate control params(iwpriv setrcparams) and STA Rx MCS dont match \n");
		return -EINVAL;
	    }
    ...
    if ((IEEE80211_IS_CHAN_11AC(ic->ic_curchan) || (IEEE80211_IS_CHAN_11NG(ic->ic_curchan))) &&
		...
		adf_os_print("********** FW VHT rate control paramters(iwpriv setrcparams) and STA rx mcs dont match\n");
		return -EINVAL;
	    }
            ...
            {
                if(!dedicated_client_fg)
                {
                    /*It is not dedicated client*/
                   ni->ni_mu_dedicate_fg =0;
                }else{
                    /*This dedicated client */
                   ni->ni_mu_dedicate_fg =1;
                }
            }
                          ...
                          struct ieee80211_app_ie_t* optie)
{
    ...
    *(u_int16_t *)frm = htole16(reason);
    ...
    if (ieee80211_vap_wme_is_set(vap) &&
        ...
        if (ASSOCWAR160_IS_VHT_CAP_CHANGE(vap->iv_cfg_assoc_war_160w) ||
            ...
            frm = ieee80211_add_vhtcap(frm, ni, ic, IEEE80211_FC0_SUBTYPE_ASSOC_RESP, &extractx);
        else
            frm = ieee80211_add_vhtcap(frm, ni, ic, IEEE80211_FC0_SUBTYPE_ASSOC_RESP, NULL);
    ...
    */
    ...
    if (ieee80211_vap_mbo_check(vap)) {
        if (ieee80211_vap_mbo_assoc_status(vap)){
