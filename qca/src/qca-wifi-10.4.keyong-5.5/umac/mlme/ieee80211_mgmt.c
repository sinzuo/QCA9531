    ...
    const size_t          optielen)
{
    ...
    if ((IEEE80211_IS_CHAN_11AC(ic->ic_curchan) || IEEE80211_IS_CHAN_11NG(ic->ic_curchan)) &&
        ...
        frm = ieee80211_add_vhtcap(frm, ni, ic, IEEE80211_FC0_SUBTYPE_PROBE_REQ, NULL);
    }
    ...
    if (IEEE80211_ADDR_IS_VALID(vap->iv_conf_des_bssid) &&
        ...
        if ( hdden_ssid_flag == 1)
        {
            ...
            if (vap->iv_des_ssid[0].len == 0 && vap->iv_des_ssid[0].ssid != '\0' ) {
                ...
                adf_os_print("%s %d ssid.len:%d  ssid.ssid:%s",
		...
		if (tmp == NULL) {
		    adf_os_print("%s:%d memory alloc failed \n",__func__,__LINE__);
		    return  -ENOMEM;
		}
		/* Copy the remaining beacon data to temp location */
		adf_nbuf_copy_bits(wbuf,(sizeof(struct ieee80211_frame) + tmp_pos_ssid),copy_length,tmp);
		...
		*(u_int8_t *)(pbeacon_frame + (tmp_pos_ssid - 1)) =  vap->iv_des_ssid[0].len;
		...
		if (adf_nbuf_put_tail(wbuf,vap->iv_des_ssid[0].len) == NULL) {
		    adf_os_print("No enough tailroom for wbuf \n");
		    OS_FREE(tmp);
		    tmp  = NULL;
		    return  -ENOMEM;
		}
		/* Insert the  SSID string */
		adf_os_mem_copy(( pbeacon_frame + tmp_pos_ssid),vap->iv_des_ssid[0].ssid,vap->iv_des_ssid[0].len);
		/* Copy rest of the Beacon data */
		adf_os_mem_copy(( pbeacon_frame + tmp_pos_ssid + vap->iv_des_ssid[0].len),tmp,copy_length);
		OS_FREE(tmp);
		tmp  = NULL;
	    } else {
		adf_os_print("Require size is more than allocated \n");
		return  -ENOMEM;
	    }
        }
    }
