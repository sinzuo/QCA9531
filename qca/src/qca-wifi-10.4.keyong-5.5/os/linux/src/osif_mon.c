...
static int get_radiotap_total_len(u_int8_t ht, u_int8_t vht)
{
    int len;
...
void osif_mon_add_radiotap_header(os_if_t osif, struct sk_buff *skb, ieee80211_recv_status *rs)
{
    ...
    */
    ...
    switch (pream_type) {
        ...
        case PHY_PREAM_TYPE_VHT: /* VHT */
            ...
            break;
        default:
            adf_os_print("Unknown preamble! \n");
    }

    /* prepare space in skb for radiotap header */
    rt_total_len = get_radiotap_total_len(ht, vht);
    if (adf_nbuf_headroom(skb) < rt_total_len) {
        tskb = adf_nbuf_realloc_headroom(skb, rt_total_len);
        if(tskb!=NULL){
            adf_nbuf_free(skb);
            skb = tskb;
        } else {
            adf_os_print(" %s[%d] skb_realloc_headroom failed\n",__func__,__LINE__);
            return;
        }
    }
    rthdr = (struct ieee80211_radiotap_header *)adf_nbuf_push_head(skb, rt_total_len);
    adf_os_mem_zero(rthdr, rt_total_len);
    ...
    */
    ...
    /* RX signal */
    it_present_mask |= cpu_to_le32(BIT(IEEE80211_RADIOTAP_DBM_ANTSIGNAL));
    *p = ic->ic_get_cur_chan_nf(ic) - DEFAULT_CHAN_NOISE_FLOOR + rs->rs_rssi + DEFAULT_CHAN_REF_NOISE_FLOOR;
    p++;

    /* RX signal noise floor */
    it_present_mask |= cpu_to_le32(BIT(IEEE80211_RADIOTAP_DBM_ANTNOISE));
...
void osif_mon_add_prism_header(os_if_t osif, struct sk_buff *skb, ieee80211_recv_status *rs)
{
    ...
    if (skb_headroom(skb) < sizeof(wlan_ng_prism2_header)) {
        ...
        if(tskb!=NULL){
            ...
            adf_os_print(" %s[%d] skb_realloc_headroom failed ",__func__,__LINE__);
            return ;
        }
    }
