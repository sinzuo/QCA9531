...
$Header: //components/rel/wlanfw_halphy_tools.cnss/1.8.0/target/utf/tcmd_utf/tcmd.c#2 $
...
A_STATUS tcmd_setup_channel(wlan_tcmd_dev_t *dev, A_UINT32 freq, A_UINT32 wlanMode, A_UINT32 bandwidth, A_UINT32 freq2);
...
static void tcmd_cont_rx_begin(wlan_tcmd_dev_t *dev, TCMD_CONT_RX_ACT act,
...
static A_STATUS
tcmd_setup_channel_real(wlan_tcmd_dev_t *dev, A_UINT32 freq, A_UINT32 wlanMode, A_UINT32 bandwidth, A_BOOL doIt, A_UINT32 freq2);
...
void utfTxChannelSetup(wlan_tcmd_dev_t *dev, TCMD_CONT_TX *contTx)
{
    ...
    if (A_OK != tcmd_setup_channel(dev, contTx->freq, contTx->wlanMode, contTx->bandwidth, contTx->freq2))
        return;
...
A_UINT32 tcmd_cont_rx_cmd(wlan_tcmd_dev_t *dev, TCMD_CONT_RX *contRx)
{
    ...
    case TCMD_CONT_RX_FILTER:
    	...
    	if (tcmd_state.tcmdRxGo == TCMD_CONT_RX_TX_ON || tcmd_state.tcmdTxGo != TCMD_CONT_TX_OFF)
        {
     	...
     	tcmd_cont_rx_begin(dev, contRx->act, contRx->u.para.freq, contRx->u.para.antenna, contRx->u.para.wlanMode, contRx->u.para.bandwidth, contRx->u.para.ack, contRx->u.para.rxChain, contRx->u.para.freq2);
        break;
...
tcmd_cont_rx_begin(wlan_tcmd_dev_t *dev, TCMD_CONT_RX_ACT act, A_UINT32 freq, A_UINT32 antenna, A_UINT32 wlanMode, A_UINT32 bandwidth, A_UINT32 ack, A_UINT32 rxChain, A_UINT32 freq2)
{
    ...
    if (tcmd_state.tcmdRxGo != 2) {
    	if (A_OK != tcmd_setup_channel(dev, freq, wlanMode, bandwidth, freq2))
            return;
    }
#endif
...
A_STATUS
tcmd_setup_channel(wlan_tcmd_dev_t *dev, A_UINT32 freq, A_UINT32 wlanMode, A_UINT32 bandwidth, A_UINT32 freq2)
{
    ...
    return(tcmd_setup_channel_real(dev, freq, wlanMode, bandwidth, TRUE, freq2));
}
...
#endif
    if (!doIt) {
    if ((freq == tcmd_state.freq) && (wlanMode == tcmd_state.wlanMode) && (bandwidth == tcmd_state.bandwidth)) {
        return A_OK;
    }
    }
    // set the state
...
void utfContRxStart(wlan_tcmd_dev_t *dev, TCMD_CONT_RX *contRx)
{
    // clean stat.
    A_MEMZERO(&tcmdRxStat, sizeof(struct tcmd_rx_stat_t));

    // start cont. rx
    tcmd_cont_rx_cmd(dev, (TCMD_CONT_RX *)contRx);
}
