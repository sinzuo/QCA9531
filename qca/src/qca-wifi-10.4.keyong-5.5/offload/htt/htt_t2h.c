...
htt_t2h_msg_handler(void *context, HTC_PACKET *pkt)
{
    ...
    if (adf_nbuf_put_tail(htt_t2h_msg, pkt->ActualLength + HTC_HEADER_LEN) == NULL) {
        adf_os_print("%s: Failed to expand head for htt_t2h_msg, len %u \n", __func__, pkt->ActualLength);
        return;
    }
    ...
    case HTT_T2H_MSG_TYPE_VERSION_CONF:
        {
            pdev->tgt_ver.major = HTT_VER_CONF_MAJOR_GET(*msg_word);
            pdev->tgt_ver.minor = HTT_VER_CONF_MINOR_GET(*msg_word);
            adf_os_print(KERN_INFO"target uses HTT version %d.%d; host uses %d.%d\n",
                pdev->tgt_ver.major, pdev->tgt_ver.minor,
                HTT_CURRENT_VERSION_MAJOR, HTT_CURRENT_VERSION_MINOR);
            if (pdev->tgt_ver.major != HTT_CURRENT_VERSION_MAJOR) {
                adf_os_print("*** Incompatible host/target HTT versions!\n");
    ...
    case HTT_T2H_MSG_TYPE_MGMT_TX_COMPL_IND:
		{
			...
			if(!adf_nbuf_is_queue_empty(&pdev->mgmtbufQ)) {
...
#define MAX_MGMT_BUF_PKTS 512
			...
			if ((subtype == IEEE80211_FC0_SUBTYPE_PROBE_RESP) &&
				...
				if(adf_nbuf_queue_len(&pdev->mgmtbufQ) > MAX_MGMT_BUF_PKTS/2) {
					ret = -1;
					goto skip_send;
				}
			}
            ...
            /*keep consistent as in ol_txrx_mgmt_send*/
    ...
    case HTT_T2H_MSG_TYPE_EN_STATS:
        {
            ...
            ol_ath_smart_ant_stats_handler(pdev->txrx_pdev, msg_word, adf_nbuf_len(htt_t2h_msg));
#endif
#if ENHANCED_STATS
            ol_ath_enh_stats_handler(pdev->txrx_pdev, msg_word, adf_nbuf_len(htt_t2h_msg));
#endif
            break;
        }
    ...
    case HTT_T2H_MSG_TYPE_TX_MODE_SWITCH_IND:
        {
            ...
            if(mode_enable) {
                ...
                if(mode == HTT_TX_MODE_PUSH_NO_CLASSIFY) {
                    ...
                    adf_os_print("Switching to Tx Mode-%u\n", mode);
                    ...
                    adf_os_print("Switching to Tx Mode-%u Threshold %d\n", mode, threshold);
        ...
        uint32_t num_cmpls, uint32_t *num_htt_cmpls)
{
    ...
    *num_htt_cmpls = 0;
    ...
    if(*num_htt_cmpls) {
        ...
        CE_update_tx_ring(pdev->txrx_pdev->ce_tx_hdl,*num_htt_cmpls);
#endif
#if !PEER_FLOW_CONTROL
