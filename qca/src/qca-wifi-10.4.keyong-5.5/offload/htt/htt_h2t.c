...
htt_h2t_send_complete(void *context, HTC_PACKET *htc_pkt)
{
     ...
     */
    adf_nbuf_set_sendcompleteflag(netbuf, TRUE);
...
htt_h2t_frag_desc_bank_cfg_msg(struct htt_pdev_t *pdev)
{
     ...
     */
    if (adf_nbuf_put_tail(msg, sizeof(struct htt_tx_frag_desc_bank_cfg_t)) == NULL) {
        adf_os_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_FRAG_DESC_BANK_CFG msg\n", __func__);
        return A_ERROR; /* failure */
    }
    ...
    *msg_word = 0;
    ...
    adf_os_print("%s - HTT_H2T_MSG_TYPE_FRAG_DESC_BANK_CFG sent to FW for radio ID = %d\n",__func__,scn->radio_id);
    return A_OK;
}
...
htt_h2t_ver_req_msg(struct htt_pdev_t *pdev)
{
     ...
     * The contribution from the HTC header is added separately inside HTC.
     */
    if (adf_nbuf_put_tail(msg, HTT_VER_REQ_BYTES) == NULL) {
        adf_os_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_VERSION_REQ msg\n", __func__);
        return A_ERROR; /* failure */
    }
...
htt_h2t_rx_ring_cfg_msg_ll(struct htt_pdev_t *pdev)
{
     ...
     * The contribution from the HTC header is added separately inside HTC.
     */
    if (adf_nbuf_put_tail(msg, HTT_RX_RING_CFG_BYTES(1)) == NULL) {
        adf_os_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_RX_RING_CFG msg\n", __func__);
        return A_ERROR; /* failure */
    }
    ...
    u_int32_t vdev_id)
{
    ...
    /* set the length of the message */
    if (adf_nbuf_put_tail(msg, HTT_H2T_STATS_REQ_MSG_EXTN1_SZ) == NULL) {
        adf_os_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_STATS_REQ msg\n", __func__);
        return -1;
    }
    ...
    *msg_word = 0;
    ...
    if (vdev_id){
        HTT_H2T_STATS_HEADER_EXTN_FLG_SET(*msg_word, PER_VAP_FW_STATS_REQUEST);
    } else {
...
#endif
    {
        status = CE_send_single(txrx_pdev->ce_tx_hdl, msg,
                                 pdev->htc_endpoint, adf_nbuf_len(msg));
    }
...
htt_h2t_sync_msg(struct htt_pdev_t *pdev, u_int8_t sync_cnt)
{
    ...
    /* set the length of the message */
    if (adf_nbuf_put_tail(msg, HTT_H2T_SYNC_MSG_SZ) == NULL) {
        adf_os_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_SYNC msg\n", __func__);
        htt_htc_pkt_free(pdev, pkt);
        return A_NO_MEMORY;
    }
                     ...
                     int max_subfrms_amsdu)
{
    ...
    /* set the length of the message */
    if (adf_nbuf_put_tail(msg, HTT_AGGR_CFG_MSG_SZ) == NULL) {
        adf_os_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_AGGR_CFG msg\n", __func__);
        return -1;
    }
    ...
    *msg_word = cookie ;
...
#endif
    {
        ...
        htt_pkt_buf_list_add(pdev,(void *)msg,cookie, NULL);
        HTT_TX_PDEV_LOCK(&txrx_pdev->tx_lock);
        status = CE_send_single(txrx_pdev->ce_tx_hdl, msg, pdev->htc_endpoint,
                adf_nbuf_len(msg));
        HTT_TX_PDEV_UNLOCK(&txrx_pdev->tx_lock);
...
#define MAX_MGMT_BUF_PKTS 512 /*TODO: This needs fine tuning */
    ...
    if (pdev->nss_wifiol_ctx) {
        ...
        status = CE_send_single(pdev->ce_tx_hdl, msg, hpdev->htc_endpoint, adf_nbuf_len(msg));
    }
#else
     status = CE_send_single(pdev->ce_tx_hdl, msg, hpdev->htc_endpoint, adf_nbuf_len(msg));
#endif
                ...
                A_UINT32             len)
{
    ...
    adf_nbuf_init(msg);
    if (adf_nbuf_put_tail(msg, (sizeof(struct htt_tx_fetch_resp_t) - 8)) == NULL) {
        adf_os_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_TX_FETCH_RESP msg\n", __func__);
        return NULL;
    }

	adf_os_mem_zero(msg->data, adf_nbuf_len(msg));
	fetch_desc->msg_buf     = msg;

    /* fill in the message contents */
    tx_fetch_resp_msg = (struct htt_tx_fetch_resp_t *) adf_nbuf_data(msg);
...
htt_h2t_fetch_resp_tx(struct ol_txrx_pdev_t *pdev,
        struct htt_tx_fetch_desc_buf *fetch_desc)
{
    adf_nbuf_t msg = fetch_desc->msg_buf;
    int status = 0;
