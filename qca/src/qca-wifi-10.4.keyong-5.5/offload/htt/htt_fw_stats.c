    ...
    wlan_dbg_tx_rate_info_t *tx_rate_info, int concise)
{
    ...
    /* MCS count SU */
    adf_os_print("MCS counts SU (0..9): ");
    adf_os_print("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
                  tx_rate_info->mcs[0],
                  tx_rate_info->mcs[1],
                  tx_rate_info->mcs[2],
    ...
    /* MCS count MU */
    adf_os_print("MCS counts MU (0..9): ");
    adf_os_print("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
        ...
        wlan_dbg_tx_pf_sched_stats_t *prefetch, int concise)
{
    ...
    for(i=0; i < WAL_STATS_PREFETCH_MAX_QUEUES; i++)
    {
        ...
        adf_os_print(" Pfsched Peer Skipped   ==> %u\n", prefetch->pfsched_peer_skipped[i]);
    }
}
...
{
    adf_os_print("\n ### TX extended stats ### \n");
    adf_os_print("tx_de_cache_miss           %u\n",tx_path_stats->tx_de_cache_miss);
    adf_os_print("tx_enqueue_peer_invalid    %u\n",tx_path_stats->tx_enqueue_peer_invalid);
    adf_os_print("tx_burst_disable_cong_ctrl %u\n",tx_path_stats->tx_burst_disable_cong_ctrl);
    adf_os_print("tx_de_lookup_failed        %u\n",tx_path_stats->tx_de_lookup_failed);
    adf_os_print("tx_delay_proc              %u\n",tx_path_stats->tx_delay_proc);
    adf_os_print("tt_prefetch_suspend        %u\n",tx_path_stats->tt_prefetch_suspend);
    adf_os_print("short_pkt                  %u\n",tx_path_stats->short_pkt);
    adf_os_print("total_enqueue              %u\n",tx_path_stats->total_enqueue);
    adf_os_print("peer_id_invalid            %u\n",tx_path_stats->peer_id_invalid);
    adf_os_print("host_inspect               %u\n",tx_path_stats->host_inspect);
    adf_os_print("pdev_zero_discard          %u\n",tx_path_stats->pdev_zero_discard);
    adf_os_print("tickle_proc_sched          %u\n",tx_path_stats->tickle_proc_sched);
    adf_os_print("residue_cleanup            %u\n",tx_path_stats->residue_cleanup);
}
