...
#define MAX_PROTO_VAL       4
        ...
        {
            {
                ...
                /* non TCP/UDP packet */
                { ADF_NBUF_RX_CKSUM_TYPE_NONE, ADF_NBUF_RX_CKSUM_TYPE_NONE },
                /* TCP packet */
    ...
    adf_nbuf_rx_cksum_t cksum = {
        ...
        ADF_NBUF_RX_CKSUM_TYPE_NONE,
        0
    } ;
    if (cksum.l4_type != ADF_NBUF_RX_CKSUM_TYPE_NONE) {
...
static u_int32_t _##target##_rx_desc_check_desc_mgmt_type(adf_nbuf_t head_msdu) \
{ \
    struct rx_desc_base *cbf_rx_desc; \
    adf_nbuf_t next_msdu = head_msdu; \
    int        msdu_chained = 0; \
    ...
    { \
        ...
        { \
            ...
            next_msdu = adf_nbuf_next(next_msdu); \
...
static u_int32_t _##target##_rx_desc_check_desc_ctrl_type(adf_nbuf_t head_msdu) \
{ \
    struct rx_desc_base *cbf_rx_desc; \
    adf_nbuf_t next_msdu = head_msdu; \
    int        msdu_chained = 0; \
        ...
        u_int32_t* npackets)
{
    ...
    adf_os_assert(ar->rx_callbacks->rx_ring_elems(ar->htt_handle) != 0);
    ...
    while (1) {
         ...
         */
        adf_nbuf_set_pktlen(msdu, rx_buf_size);

        adf_nbuf_unmap_nbytes(ar->osdev, msdu, ADF_OS_DMA_FROM_DEVICE, rx_buf_size);
        ...
        rssi_chain0_sec80= (((*(u_int32_t *) &rx_desc->ppdu_start) &
            ...
            adf_os_ath_gen_randomness(rssi_chain0_pri20 | rssi_chain0_sec80);
        }
#endif
        ...
        while (msdu_chained--) {
            ...
            adf_nbuf_set_pktlen(next, rx_buf_size);
            adf_nbuf_unmap_nbytes(ar->osdev, next, ADF_OS_DMA_FROM_DEVICE, rx_buf_size);
...
static inline adf_nbuf_t unified_rx_desc_restitch_mpdu_from_msdus(ar_handle_t arh,
        adf_nbuf_t head_msdu,
        ...
        unsigned clone_not_reqd)
{
     ...
     */
    clone_nbuf_fn =
        clone_not_reqd ? rx_adf_noclone_buf : adf_nbuf_clone;
...
#define MAX_MONITOR_HEADER (512)
    mpdu_buf = adf_nbuf_alloc(ar->osdev,
    ...
    while (msdu_orig) {
        ...
        if(is_first_frag)
        {
                ...
                amsdu_llc_buf = adf_nbuf_alloc(ar->osdev,
                    32 + 32,
                    32, 4, FALSE);
                ...
                dest = adf_nbuf_put_tail(amsdu_llc_buf, msdu_llc_len + amsdu_pad);
                if (!dest) {
                    goto mpdu_stitch_fail;
                }
                dest += amsdu_pad;
                ...
                adf_nbuf_set_next_ext(prev_buf, amsdu_llc_buf);
...
static adf_nbuf_t _##target##_rx_desc_restitch_mpdu_from_msdus(ar_handle_t arh, \
        adf_nbuf_t head_msdu, \
        struct ieee80211_rx_status *rx_status, \
        unsigned clone_not_reqd) \
{ \
...
static inline void unified_rx_desc_get_vowext_stats(adf_nbuf_t msdu,
...
static void _##target##_rx_desc_get_vowext_stats(adf_nbuf_t msdu, \
        struct vow_extstats *vowstats) \
{ \
...
static inline int unified_rx_desc_update_pkt_info(adf_nbuf_t head_msdu,
                                                  adf_nbuf_t tail_msdu,
                                                  uint32_t permsdu_flag)
{
    ...
    if (!permsdu_flag || (is_head)) {
        /* store frag value before overwriting skb-cb */
        frag = adf_nbuf_is_chfrag_start(head_msdu);
        first_mpdu =
                 ((*((u_int32_t *) &rx_desc->attention)) &
                  RX_ATTENTION_0_FIRST_MPDU_MASK) >>
                  RX_ATTENTION_0_FIRST_MPDU_LSB;
        if (first_mpdu) {
#if ATH_DATA_RX_INFO_EN
            if(!(rx_status = adf_os_mem_alloc(NULL, sizeof(struct per_msdu_data_recv_status)))) {
                return -1;
            }
            adf_nbuf_set_fctx_type(head_msdu, (u_int32_t)rx_status, CB_FTYPE_RX_INFO);
#else
            if (!(rx_status = adf_os_mem_alloc(NULL, sizeof(struct per_msdu_mesh_data_recv_status)))) {
                return -1;
            }
            adf_nbuf_set_fctx_type(head_msdu, (u_int32_t)rx_status, CB_FTYPE_MESH_RX_INFO);
#endif
        ...
        adf_nbuf_set_chfrag_start(head_msdu,frag);
    }
    ...
    if (!permsdu_flag || (is_tail)) {
        ...
        if (last_mpdu) {
            if (!rx_status) {
                ...
                adf_nbuf_set_fctx_type(tail_msdu, (u_int32_t)rx_status, CB_FTYPE_RX_INFO);
#else
                if (!(rx_status = adf_os_mem_alloc(NULL, sizeof(struct per_msdu_mesh_data_recv_status)))) {
                    return -1;
                }
                adf_nbuf_set_fctx_type(tail_msdu, (u_int32_t)rx_status, CB_FTYPE_MESH_RX_INFO);
#endif
            }
