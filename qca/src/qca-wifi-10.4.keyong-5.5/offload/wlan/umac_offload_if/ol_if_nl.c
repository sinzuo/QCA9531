...
wmi_unified_mu_report_event_handler(ol_scn_t scn, u_int8_t *data, u_int16_t datalen, void *context)
{
    ...
    for (i = 0; i < event->num_TA_entries; i++) {
        ...
        mu_rpt.mu_database_entries[i].mu_device_type = event->mu_entry[i].entry_type;
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_NL,
                             "Device type=%d\n",event->mu_entry[i].entry_type);

        WMI_MAC_ADDR_TO_CHAR_ARRAY(&event->mu_entry[i].bssid_mac_addr,
                                   mu_rpt.mu_database_entries[i].mu_device_bssid);
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_NL,
                             "BSSID mac address=%08x%08x\n", event->mu_entry[i].bssid_mac_addr.mac_addr47to32,
                             event->mu_entry[i].bssid_mac_addr.mac_addr31to0);

        WMI_MAC_ADDR_TO_CHAR_ARRAY(&event->mu_entry[i].tx_addr,
                                   mu_rpt.mu_database_entries[i].mu_device_macaddr);
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_NL,
                             "TA mac address=%08x%08x \n", event->mu_entry[i].tx_addr.mac_addr47to32,
                             event->mu_entry[i].tx_addr.mac_addr31to0);

        mu_rpt.mu_database_entries[i].mu_avg_duration = event->mu_entry[i].avg_duration_us;
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_NL,
                             "Average duration=%d\n", event->mu_entry[i].avg_duration_us);

        mu_rpt.mu_database_entries[i].mu_avg_rssi = event->mu_entry[i].avg_rssi;
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_NL,
                             "Average RSSI=%d\n", event->mu_entry[i].avg_rssi);

        mu_rpt.mu_database_entries[i].mu_percentage = event->mu_entry[i].mu_percent;
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_NL,
                             "MU in percentage=%d\n", event->mu_entry[i].mu_percent);
    }
               ...
               u_int32_t home_plmnid, u_int32_t alpha_active_bssid)
{
    ...
    cmd->rssi_thr_bssid = thr_bssid;
    cmd->rssi_thr_sta = thr_sta;
    cmd->rssi_thr_sc = thr_sc;
    cmd->plmn_id = home_plmnid & 0xFFFFFF;
    cmd->alpha_num_bssid = alpha_active_bssid;
                   ...
                   ieee80211req_lteu_cfg_t *config, u_int32_t wifi_tx_power)
{
    ...
    cmd->allow_err_packets = config->lteu_cfg_reserved_1;
    return wmi_unified_cmd_send(scn->wmi_handle, buf,
