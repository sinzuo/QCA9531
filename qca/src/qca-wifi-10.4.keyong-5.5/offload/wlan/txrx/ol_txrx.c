...
ol_tx_pflow_ctrl_init(struct ol_txrx_pdev_t *pdev)
{
    ...
    adf_os_mem_zero(pdev->pflow_ctl_next_peer_idx, 4 * OL_TX_PFLOW_CTRL_MAX_TIDS);
#endif
    adf_os_mem_zero(pdev->pflow_ctl_active_peer_map,
    ...
    adf_os_timer_init(pdev->osdev, &pdev->pflow_ctl_cong_timer,
            ol_tx_pflow_ctrl_cong_ctrl_timer, (void *)pdev);

    adf_os_timer_mod(&pdev->pflow_ctl_cong_timer,
            OL_TX_PFLOW_CTRL_CONG_CTRL_TIMER_MS);

    adf_os_timer_init(pdev->osdev, &pdev->pflow_ctl_stats_timer,
            ol_tx_pflow_ctrl_stats_timer, (void *)pdev);
    ...
    adf_os_print("Startup Mode-%d set\n", pdev->pflow_ctrl_mode);
...
ol_tx_pflow_ctrl_clean(struct ol_txrx_pdev_t *pdev)
{
    ...
    adf_os_timer_free(&pdev->pflow_ctl_cong_timer);
    adf_os_timer_free(&pdev->pflow_ctl_stats_timer);
}
...
ol_txrx_mempools_attach(ol_pdev_handle ctrl_pdev)
{
    ...
    if (adf_mempool_init(scn->adf_dev, &scn->mempool_ol_ath_peer,
    ...
    adf_os_device_t osdev)
{
    ...
    adf_nbuf_queue_init(&pdev->acnbufq);
    adf_os_spinlock_init(&pdev->acnbufqlock);
    ...
    pdev->osdev = osdev;
    pdev->targetdef = sc->targetdef;

#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    printk("pdev attach %p %d \n",sc->nss_wifiol_ctx,sc->nss_wifiol_id);
    if (sc->nss_wifiol_ctx){
        pdev->nss_wifiol_id = sc->nss_wifiol_id;
        pdev->nss_wifiol_ctx = sc->nss_wifiol_ctx;
    ...
    pdev->htt_pdev = htt_attach(
        pdev, ctrl_pdev, htc_pdev, scn->arh, osdev, desc_pool_size);
    if (!pdev->htt_pdev) {
        goto fail2;
    }
    ...
    adf_os_print(KERN_INFO"%s: %d tx desc's allocated ; range starts from %p\n",
    ...
    for (i = 0; i < desc_pool_size; i++) {
        ...
        if (! htt_frag_desc) {
            adf_os_print("%s: failed to alloc HTT frag desc (%d of %d)\n",
                    __func__, i, desc_pool_size);
            goto fail4;
        }
    ...
    if (pdev->nss_wifiol_ctx) {
        ...
        if (osif_nss_ol_pdev_attach(sc, pdev->nss_wifiol_ctx, pdev->nss_wifiol_id,
    ...
    pdev->tx_tso_desc.array = adf_os_mem_alloc(
		    osdev, (desc_pool_size/2) * sizeof(union ol_tx_tso_desc_list_elem_t));
    if (!pdev->tx_tso_desc.array) {
	    goto fail4;
    }

    adf_os_mem_set(
		    pdev->tx_tso_desc.array, 0,
    ...
    pdev->tx_sg_desc.array = adf_os_mem_alloc(
		    osdev, (desc_pool_size) * sizeof(union ol_tx_sg_desc_list_elem_t));
    if (!pdev->tx_sg_desc.array) {
	    goto fail4;
    }

    adf_os_mem_set(
		    pdev->tx_sg_desc.array, 0,
...
fail5:
#if (HOST_SW_TSO_ENABLE || HOST_SW_TSO_SG_ENABLE)
        adf_os_mem_free(pdev->tx_tso_desc.array);
#endif /* HOST_SW_TSO_ENABLE || HOST_SW_TSO_SG_ENABLE */
#if HOST_SW_SG_ENABLE
        adf_os_mem_free(pdev->tx_sg_desc.array);
#endif /* HOST_SW_SG_ENABLE */
...
fail1:
    adf_os_spinlock_destroy(&pdev->acnbufqlock);
#if PEER_FLOW_CONTROL
...
ol_txrx_pdev_detach(ol_txrx_pdev_handle pdev, int force)
{
    ...
    adf_os_timer_free(&pdev->pflow_ctl_stats_timer);
#if PEER_FLOW_CONTROL_HOST_SCHED
    adf_os_timer_cancel(&pdev->pflow_ctl_dequeue_timer);
    adf_os_timer_free(&pdev->pflow_ctl_dequeue_timer);
#endif
#endif
...
/* USE vdev->pdev->osdev, AND REMOVE PDEV FUNCTION ARG? */
    peer = (struct ol_txrx_peer_t *)adf_mempool_alloc(scn->adf_dev, scn->mempool_ol_ath_peer);
...
#endif
    {
        for (i = 0; i < OL_TX_PFLOW_CTRL_HOST_MAX_TIDS; i++) {
            ...
            adf_nbuf_queue_init(&peer->tidq[i].nbufq);
        }
    }
#endif
...
ol_txrx_peer_unref_delete(ol_txrx_peer_handle peer)
{
    ...
    if (adf_os_atomic_dec_and_test(&peer->ref_cnt)) {
        ...
        adf_mempool_free(scn->adf_dev, scn->mempool_ol_ath_peer, peer);
 ...
 */
    ...
    if (req->wait.blocking) {
        ...
        mutex_ret = adf_os_mutex_acquire_timeout(pdev->osdev, &scn->scn_stats_sem, 3000);
    }
    ...
    struct ol_txrx_stats_req *req)
{
    ...
    adf_os_print("### HOST MSDU TTL Stats ###\nHost_msdu_ttl     :\t%d\n",pdev->pflow_msdu_ttl_cnt);
    return 0;
}
#endif
    ...
    struct ol_txrx_stats_req *req)
{
    ...
    adf_os_print("Ol Tx Desc In Use\t:  %u\n", stats->tx.desc_in_use);
    adf_os_print("Ol Tx Desc Failed\t:  %u\n", stats->tx.desc_alloc_fails);
    adf_os_print("CE Ring (4) Full \t:  %u\n", stats->tx.ce_ring_full);
    adf_os_print("DMA Map Error    \t:  %u\n", stats->tx.dma_map_error);
    ...
    adf_os_print("++++++++++ HOST FLOW CONTROL STATISTICS +++++++++++\n");
    adf_os_print("Receive from stack count: %u\n", stats->tx.from_stack.pkts);
    adf_os_print("non queued pkt count: %u\n", stats->tx.fl_ctrl.fl_ctrl_avoid);
    adf_os_print("queued pkt count: %u\n", stats->tx.fl_ctrl.fl_ctrl_enqueue);
    adf_os_print("queue overflow count: %u\n", stats->tx.fl_ctrl.fl_ctrl_discard);
    return 0;
}
void
    ...
    ol_txrx_vdev_handle vdev)
{
    ...
    /* HACK */
    adf_os_print("++++++++++ HOST MCAST Ehance STATISTICS +++++++++++\n");
    adf_os_print("Mcast recieved\t: %d\n", stats->mcast_enhance.num_me_rcvd);
    adf_os_print("ME converted\t: %d\n", stats->mcast_enhance.num_me_ucast);
    adf_os_print("ME dropped (Map)\t: %d\n", stats->mcast_enhance.num_me_dropped_m);
    adf_os_print("ME dropped (alloc)\t: %d\n", stats->mcast_enhance.num_me_dropped_a);
    adf_os_print("ME dropped(internal)\t: %d\n", stats->mcast_enhance.num_me_dropped_i);
    adf_os_print("ME dropped(own address)\t: %d\n", stats->mcast_enhance.num_me_dropped_s);
    adf_os_print("ME bufs in use\t: %d\n", stats->mcast_enhance.num_me_buf);
    adf_os_print("ME bufs in non pool allocation in use\t: %d\n", stats->mcast_enhance.num_me_nonpool);
    adf_os_print("ME bufs in non pool allocation\t: %d\n", stats->mcast_enhance. num_me_nonpool_count);
    adf_os_print("\n");
...
#define STATS_LENGTH (4 * 1024)
                {
                    stats_wqp = adf_os_mem_alloc(pdev->osdev, sizeof(adf_os_work_t));
                    if(!stats_wqp) {
                        htt_t2h_stats_print(stats_info_list, req->base.print.concise, scn->target_type, vdev_id);
                        break;
                    }
                    stats_buffer = adf_os_mem_alloc(pdev->osdev, STATS_LENGTH);
                    if(!stats_buffer) {
                        htt_t2h_stats_print(stats_info_list, req->base.print.concise, scn->target_type, vdev_id);
                        adf_os_mem_free(stats_wqp);
                        break;
                    }
                    ...
                    *(uint32_t *)((uint8_t *)stats_buffer + 8) = (uint32_t)vdev_id;
                    adf_os_create_work(pdev->osdev, stats_wqp,
                            stats_deferred_work, stats_buffer);
                    adf_os_mem_copy(((uint8_t *)stats_buffer + 12), stats_info_list, length);
                    adf_os_sched_work(pdev->osdev, stats_wqp);
                }
#else
                htt_t2h_stats_print(stats_info_list, req->base.print.concise, scn->target_type, vdev_id);
#endif
		...
		ol_txrx_vdev_handle vdev)
{
	...
	adf_os_print("++++++++++ RX Checksum Error STATISTICS +++++++++++\n");
	adf_os_print(
...
ol_rst_rx_cksum_stats(ol_txrx_vdev_handle vdev)
{
	...
	adf_os_print(".....Resetting RX Checksum Error Stats \n");
	/* Rx */
