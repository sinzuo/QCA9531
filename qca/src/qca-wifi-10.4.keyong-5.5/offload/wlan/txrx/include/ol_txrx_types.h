...
static inline
int ol_txrx_is_dhcp(adf_nbuf_t nbf, struct iphdr *ip)
{
    ...
    if (ip->protocol == IP_PROTO_UDP)
    {
        ...
        udpSrc = adf_os_htons(udp->source);
        udpDst = adf_os_htons(udp->dest);
...
struct ol_txrx_pdev_t {
...
#endif
    adf_nbuf_queue_t acnbufq;
    u_int32_t acqcnt;
    u_int32_t acqcnt_len;
    adf_os_spinlock_t acnbufqlock;
    ...
    adf_os_timer_t pflow_ctl_cong_timer;

    /* Stats Display timer */
    /* @todo Change to Workqueue */
    adf_os_timer_t pflow_ctl_stats_timer;

    u_int32_t pflow_ctl_stats_queue_depth;

#if !QCA_OL_TX_PDEV_LOCK
    adf_os_spinlock_t peer_lock[OL_TXRX_MAX_PEER_IDS];
#endif
    ...
    adf_nbuf_t mon_mpdu_skb_list_head;
    adf_nbuf_t mon_mpdu_skb_list_tail;
#endif
