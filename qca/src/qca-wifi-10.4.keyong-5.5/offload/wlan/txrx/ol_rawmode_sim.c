...
static int encap_eth_to_dot11(
    adf_nbuf_t nbuf, enum ieee80211_opmode mode,u_int8_t *bssid,struct ieee80211vap *vap,struct ol_txrx_vdev_t *vdev);
                             ...
                             struct ol_txrx_peer_t *peer)
{
    ...
    if ((msdulen < sizeof(struct llc)) ||
        ...
        adf_os_print("Invalid MSDU length %hu", msdulen);
#endif
...
decap_dot11_to_8023(adf_nbuf_t mpdu, struct ol_txrx_peer_t *peer)
{
    ...
    memset(&wh, 0, sizeof(wh));
    adf_os_mem_copy(&wh, adf_nbuf_data(mpdu), sizeof(struct ieee80211_frame));
    type = wh.i_fc[0] & IEEE80211_FC0_TYPE_MASK;
    subtype = wh.i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
    is_4addr = ((wh.i_fc[1] & IEEE80211_FC1_DIR_MASK) ==
                    IEEE80211_FC1_DIR_DSTODS) ? 1:0;
    wh_ptr = (struct ieee80211_qosframe_addr4 *)adf_nbuf_data(mpdu);
    ...
    if (is_4addr) {
        ...
        /* We don't need to bother about whether the frame is QoS, here.
           ...
           the field. */
        adf_os_mem_copy(&wh.i_addr4[0], wh_ptr->i_addr4, IEEE80211_ADDR_LEN);
    }
    ...
    adf_nbuf_t nbuf, enum ieee80211_opmode mode, u_int8_t *bssid,struct ieee80211vap *vap,struct ol_txrx_vdev_t *vdev)
{
    ...
    if ((IEEE80211_VAP_IS_PRIVACY_ENABLED(vap)) && !send_clr_text)  {
        ...
        adf_os_mem_set(adf_nbuf_data(nbuf), 0, hdrsize);
        if (adf_nbuf_put_tail(nbuf, hdrsize) == NULL) {
            adf_os_print("### Raw Mode simulation encap: No headroom for 80211 "
                         "MIC Tail\n");
            return -1;
        }
    ...
    if (!IEEE80211_IS_MULTICAST(eth_hdr.ether_dhost)) {
        ...
        if (addr4_frame) {
            wh_4addr = (struct ieee80211_frame_addr4 *) adf_nbuf_data(nbuf);
        }
        if(!noqos) {
