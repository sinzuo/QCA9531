...
extern void ol_tx_me_convert_ucast_partner(ol_txrx_vdev_handle vdev, adf_nbuf_t wbuf,
...
ol_tx_me_alloc_descriptor(struct ol_txrx_pdev_t *pdev)
{
    if(atomic_read(&pdev->mc_num_vap_attached) == 0){
        ol_tx_me_init(pdev);
        adf_os_print("Enable MCAST_TO_UCAST\n");
    }
...
ol_tx_me_free_descriptor(struct ol_txrx_pdev_t *pdev)
{
    ...
    if(atomic_read(&pdev->mc_num_vap_attached) == 0) {
        ...
        ol_tx_me_exit(pdev);
        adf_os_print("Disable MCAST_TO_UCAST\n");
    }
}
...
#if !MCAST_ENHANCE_NO_CONSISTENT_MEM 
        pdev->me_buf.vaddr = adf_os_mem_alloc_consistent( pdev->osdev, pool_size, &pool_paddr,
                adf_os_get_dma_mem_context((&pdev->me_buf), memctx));
        pdev->me_buf.paddr = pool_paddr;
#else
        pdev->me_buf.vaddr = adf_os_mem_alloc( pdev->osdev, pool_size );
...
ol_tx_me_exit(struct ol_txrx_pdev_t *pdev)
{
    ...
    if (pdev->me_buf.vaddr) {
        ...
        adf_os_mem_free_consistent( pdev->osdev,
                ...
                adf_os_get_dma_mem_context((&pdev->me_buf), memctx));
#else
        adf_os_mem_free( pdev->me_buf.vaddr);
#endif /*MCAST_ENHANCE_NO_CONSISTENT_MEM*/
                             ...
                             u_int8_t newmac[][6], uint8_t new_mac_cnt)
{
    ...
    len = adf_nbuf_len( wbuf );
    dwnld_len = pdev->me_buf.size;
    len = len < dwnld_len ? len : dwnld_len;

    data = adf_nbuf_data( wbuf );

    if(A_STATUS_FAILED == adf_nbuf_map_single( pdev->osdev, wbuf, ADF_OS_DMA_TO_DEVICE)) {
        adf_nbuf_free(wbuf);
        TXRX_STATS_ADD(pdev, pub.mcast_enhance.num_me_dropped_m, 1);
        return 1;
    }
    /* Set Null to begin with */
    adf_nbuf_set_fctx_type( (adf_nbuf_t) wbuf, 0, CB_FTYPE_MCAST2UCAST);
    ...
    for( new_mac_idx = 0; new_mac_idx < new_mac_cnt; new_mac_idx++ ) {
        ...
        if( new_mac_idx < (new_mac_cnt - 1) ) {
            wbuf_cpy = adf_nbuf_clone((adf_nbuf_t)wbuf);
            if( wbuf_cpy == NULL ) {
                TXRX_STATS_ADD(pdev, pub.mcast_enhance.num_me_dropped_a, 1);
        ...
        adf_os_mem_copy( &mc_uc_buf->buf[0], data, len );
        eh2 = (struct ether_header *)&mc_uc_buf->buf[0];

        /* copy new dest addr */
        IEEE80211_ADDR_COPY(&eh2->ether_dhost[0], dstmac);

        adf_nbuf_set_fctx_type( (adf_nbuf_t) wbuf_cpy, (u_int32_t) mc_uc_buf,
