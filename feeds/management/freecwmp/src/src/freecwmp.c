...
#include <time.h>  //added by pixiaocong in 2017-05-11
...
freecwmp_netlink_interface(struct nlmsghdr *nlh)
{
	...
	lfc_log_message(NAME, L_NOTICE, "interface %s has ip %s\n",
			if_name, if_addr);
...
int main (int argc, char **argv)
{
	...
	int c;
	while (1) {
		c = getopt_long(argc, argv, "fhv", long_opts, NULL);
		if (c == EOF)
			break;
		switch (c) {
			case 'f':
	...
	lfc_log_message(NAME, L_NOTICE, "entering main loop\n");
	uloop_run();
	...
	lfc_log_message(NAME, L_NOTICE, "exiting\n");
	return 0;
}
