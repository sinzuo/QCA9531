...
function action_flashops()
	...
	local function storage_size()
        ...
        local function check_filename(fname)
            if not fname then return false end
            local ev="/etc/version"
            local hwver=luci.util.exec("test -f "..ev.." && cat "..ev.." | tr -d \"\n\"")
            if not hwver or "" == string.gsub(hwver," ","") then return true end
            if string.find(fname,"^"..hwver) then return true end
            if "firmware_update.bin" == fname then return true end
            return false
        end 
		...
		function(meta, chunk, eof)
			if not fp then
				if meta and meta.name == "image" then
                                        isfile = check_filename(meta.file)       
		...
		if step == 1 then
                    ...
                    if not isfile then
                        luci.template.render("admin_system/flashops", {                                                       
                                        reset_avail   = reset_avail,                                                                  
                                        upgrade_avail = upgrade_avail,                                                                
                                        filename_invail = true                                                                          
                                })       
                        return nil 
                    end
			if image_supported() and image_checksum() == md5 then
				luci.template.render("admin_system/upgrade", {
					...
					keep     = (not not luci.http.formvalue("keep"))
                        ...
                        elseif image_supported() and image_checksum() ~= md5 then
                                luci.template.render("admin_system/checkfailed", {                            
					checksum = image_checksum(),
					md5 = md5,
					storage  = storage_size(),
					size     = (fs.stat(image_tmp, "size") or 0),
