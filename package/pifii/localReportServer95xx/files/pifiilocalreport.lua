...
function _M.get_confversion()
    uci:load("localreport")
    return uci:get("localreport", "confinfo", "conf_version") or ""
end
...
function _M.get_manufacturer()
    local res = uci:get_first("localreport","device","factory","")
    --if "" == res then
...
function _M.get_ap_model()
    local res = util.exec("test -f /etc/version && cat /etc/version | tr -d '\n'")
    if "" == res then
        res = util.exec("cat /etc/openwrt_release | grep DISTRIB_CODENAME | tr -d \"DISTRIB_CODENAME=|\\'|\\n\" | tr -s \'[a-z]\' \'[A-Z]\'")
    end
    return res
end
...
function _M.get_rx_tx()
    local ifname = uci:get("network","wan","ifname") or "br-lan"
    local bytes = util.exec(string.format([[ifconfig %s | tail -n 2 | grep -v -E "^$" | awk '{print$3$4" "$7$8}' | tr -d "\(|\)"]],ifname))
    bytes = string.gsub(bytes,"iB","")
    local bt = _M.split(bytes," ")
    return bt[1],bt[2]
end
