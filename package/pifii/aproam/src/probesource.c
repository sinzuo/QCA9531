...
int probe_source_init()
{
	...
	if((fp=fopen("/tmp/memaddr","r"))==NULL)
        {
	    fprintf(stderr,"/tmp/memaddr file cannot be opened");
	    return -1;
	}

	if(4!=fread(addr,1,4,fp))
	{
	    fprintf(stderr,"/tmp/memaddr read error count");
	    fclose(fp);
	    return -1;
	}
	fclose(fp);
	...
	//printf"%X \n",phy_addr);
        ...
        {
            //perror("open");
	    fprintf(stderr,"/dev/mem file cannot be opened");
	    return -1;
        }
...
uint32_t probe_source_read(uint8_t *mactable,uint32_t size)
{
        if (NULL == g_map_buf)
        {
            fprintf(stderr,"error:probe source data not init");
            return 0;
        }
