...
int mtd_write( stumac_t *pstmac,int op)
{
    int sz = 0;
    int i;
    unsigned char pre[4] = {0xd8,0xd8,0x66,'\0'};
    int wflag = 0;
    if(0 == op) 
    {
        if(0 != memcmp(pstmac->mac,pre,3))
        {
            fprintf(stderr,"must input sztozed's device mac,D8:D8:66:XX:XX:XX\n");
            return -1; 
        }
    }
    ...
    */
    ...
    if(0 == op) 
    {
        if(0 != memcmp(ptr,pre,3))
        {
            fprintf(stderr,"not sztozed device mac in art!\n");
            goto write_fail;
        }
    }
    if(0 != memcmp(ptr,pstmac->mac,MACLEN_6))
    {
